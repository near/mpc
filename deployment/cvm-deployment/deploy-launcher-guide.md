...
- Launcher and MPC node manifest digests set via `LAUNCHER_MANIFEST_DIGEST` and `MPC_MANIFEST_DIGEST` (the script renders `crates/contract/assets/launcher_docker_compose.yaml.template` with them; discover currently-allowed digests with `./scripts/fetch-allowed-launcher-hashes.sh --network testnet`)
