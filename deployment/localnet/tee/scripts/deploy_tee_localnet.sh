...
CONTRACT_INITIAL_BALANCE="${CONTRACT_INITIAL_BALANCE:-20 NEAR}"
NODE_INITIAL_BALANCE="${NODE_INITIAL_BALANCE:-3.0 NEAR}"
...
# templates live here (UPDATED for move to deployment/localnet/tee/scripts)
ENV_TPL="$REPO_ROOT/deployment/localnet/tee/scripts/node.env.tpl"
if [ "$MODE" = "localnet" ]; then
  CONF_TPL="$REPO_ROOT/deployment/localnet/tee/scripts/node.conf.localnet.tpl"
else
  CONF_TPL="$REPO_ROOT/deployment/localnet/tee/scripts/node.conf.tpl"
fi
...
# Derived accounts
ACCOUNT_SUFFIX="${ACCOUNT_SUFFIX:-.testnet}"   # localnet example: ".test.near"
