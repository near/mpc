# 2025-11-17 testnet upgrade guide
These are the commands we need to run to upgrade nodes running the latest `2.2.0` release
to `3.0.3`.

# Prerequisites
The `nearone/mpc-node-gcp:testnet-release` image must point to the `3.0.3` release.
All nodes must be running the latest `2.2.0` release.
...
nearone/mpc-node-gcp    testnet-release   b47a691282f0   11 days ago   3.79GB
```
...
Now, `3.0.3` requires a few new config values:
...
Check the `3.0.3` version is running from the metrics
