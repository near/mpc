...
Set the launcher and MPC node manifest digests in the env file (`LAUNCHER_MANIFEST_DIGEST` and `MPC_MANIFEST_DIGEST`). `deploy-launcher.sh` renders the contract template at deploy time, substituting these digests into the `image:` and `DEFAULT_IMAGE_DIGEST` fields.
