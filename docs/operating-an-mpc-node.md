...
For TEE nodes, keeping the host's BIOS, CPU microcode and TDX module current is part of the job, and it isn't something we can coordinate for you: Intel raises the TCB bar on its own schedule, and a platform below it has its attestation rejected until the host is updated. Check where your host stands and update it following [TDX platform TCB status](./tdx-tcb-status.md). Doing this proactively, rather than after a demotion, is what keeps the network from losing several participants at once.
