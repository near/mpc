...
It builds upon the [Localnet Setup Guide](https://github.com/near/mpc/blob/main/docs/development/localnet/localnet.md), which describes how to launch a local blockchain network.
...
For details on how to set up a TDX-based Confidential VM and prepare the DStack environment, refer to the [Running an MPC Node in TDX External Guide](https://github.com/near/mpc/blob/main/docs/guide/running-an-mpc-node-in-tdx-external-guide/running-an-mpc-node-in-tdx-external-guide.md).

---

## High-Level Steps

1. Prepare a TDX-enabled setup as described in the [TDX Guide](https://github.com/near/mpc/blob/main/docs/guide/running-an-mpc-node-in-tdx-external-guide/running-an-mpc-node-in-tdx-external-guide.md).
2. Prepare the localnet setup as described in the [Localnet Setup Guide](https://github.com/near/mpc/blob/main/docs/development/localnet/localnet.md), excluding the startup of the MPC nodes.
...
Follow the [TDX Guide](https://github.com/near/mpc/blob/main/docs/guide/running-an-mpc-node-in-tdx-external-guide/running-an-mpc-node-in-tdx-external-guide.md). Up until (but not including **MPC Node Setup and Deployment**
)
...
Follow the [Localnet Setup Guide](https://github.com/near/mpc/blob/main/docs/development/localnet/localnet.md) up until (but not including) the **“Start MPC nodes”** section.
...
The args name `tee-verifier.test.near` as the trusted TEE verifier. Deploy it as described in
[step 6 of the localnet guide](localnet.md#6-optional-deploy-the-tee-verifier) before the nodes
attest: unlike the mock attestations used there, the Dstack attestations submitted here are
forwarded to it.
...
```
(ExecutionError("Smart contract panicked: the submitted attestation failed verification, reason: custom error: `the allowed mpc image hashes list is empty`"
```
...
near contract call-function as-transaction mpc-contract.test.near sign \
  file-args docs/development/localnet/args/sign_ecdsa.json \
