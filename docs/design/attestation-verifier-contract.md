...
Instead, those entries **age out on their own**. Every stored `VerifiedAttestation` carries an `expiry_timestamp_seconds` and [`re_verify`][re-verify] already rejects any entry once it is past — the same check that bounds a legitimate attestation's lifetime also bounds a wrongly-accepted one's. The lever is that expiry. In steady state it is stamped from a hardcoded `DEFAULT_EXPIRATION_DURATION_SECONDS` of **7 days** (stamped at attestation time and carried through the DTO into the stored entry; the contract enforces it, it doesn't compute it). The 7-day window is deliberately wide: attestation freshness depends on Intel's PCCS/PCS collateral being reachable at resubmit time, and a *global* PCCS outage lasting a day or more could otherwise expire every node at once and knock out the whole cluster. It must also stay well above the honest-node resubmit cadence so nobody is evicted for timing: `mpc-node`'s [`periodic_attestation_submission`][periodic-attestation-submission] resubmits every hour, and [`monitor_attestation_removal`][monitor-attestation-removal] resubmits the moment a node's entry disappears.

The tighter containment window is only *needed* right after a rotation — the moment we stop trusting entries the old (possibly buggy) verifier produced. So the shortening is **rotation-triggered, not permanent**: when a `vote_tee_verifier_change` crosses threshold and updates `tee_verifier_account_id`, the contract also shortens every currently stored attestation to `min(existing, now + 1 day)` (never extending one that already expires sooner). Entries submitted *after* the rotation are verified by the new verifier and stamped with the normal 7-day window again; honest nodes re-attest within the hour, well inside the shortened window. A wrongly-accepted entry therefore rides out for at most ~1 day after the rotation, while steady-state operation keeps the wide outage margin.

This is a departure from an earlier revision of this design, which lowered `DEFAULT_EXPIRATION_DURATION_SECONDS` to a permanent 1 day and claimed the containment happened "uniformly and with no sweep, flag, or per-entry bookkeeping." That is no longer accurate: the rotation cap is a one-shot, bounded rewrite over `stored_attestations`, executed once on the threshold-crossing `vote_tee_verifier_change` (its natural home). The set is bounded by the participant plus recent-submitter count, so the rewrite fits within a single transaction's gas. The alternative — an immediate purge down to threshold the instant a rotation lands — is effectively expiration zero, where a transient PCCS or connectivity hiccup can knock honest nodes out before they re-attest; the 1-day cap contains the bad entry without that cliff. One benign corner: a `submit_participant_info` whose verification was scheduled against the old verifier but *resolves after* the rotation vote is stored *after* the cap ran, so it is not shortened and ages out on the normal 7-day window; this is bounded to the tiny in-flight window (one cross-contract call), the entry is still subject to allowlist `re_verify`, and the submitting node re-attests through the new verifier within the hour.
...
The verifier-rotation design changes the test surface in four ways. First, the expiration window itself: an entry whose `expiry_timestamp_seconds` is in the past must be rejected by `re_verify` even when every post-DCAP allowlist invariant still holds, and an entry within the window must still pass — the existing expiry check, exercised against the steady-state 7-day `DEFAULT_EXPIRATION_DURATION_SECONDS`. Second, the rotation cap: when `vote_tee_verifier_change` crosses threshold, every stored attestation is shortened to `min(existing, now + 1 day)` — entries expiring later are lowered to the cap, entries already expiring sooner are left untouched (never extended) — while a `submit_participant_info` after the rotation is stamped with the full 7-day window again. Third, rotation routing: after the vote crosses threshold, the next `submit_participant_info` must call `verify_quote` on the new `tee_verifier_account_id`, and existing stored entries must remain present (no purge) until they expire. Fourth, the in-flight case: a verification scheduled against the old verifier that resolves after the vote crosses threshold is stored *after* the cap ran, so it is not shortened and ages out via the normal expiration window like any other entry.
