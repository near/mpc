...
**followers**. The generic protocol runner (`crates/node/src/protocol.rs`)
handles incoming generation requests from leaders over the P2P network.
When a leader spawns a triple or presignature protocol, each follower
receives the protocol messages, runs its side of the MPC computation,
and stores the result. All participants hold a different share of the
same asset, identified by a common unique ID.
