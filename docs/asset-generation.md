...
key and purpose (signatures, CKD, foreign transactions, etc). Triples are
not domain-specific; a single shared triple store feeds presignature
generation for all ECDSA domains. Presignatures are per-domain because
they incorporate the domain's key share.
...
- The **owner** (leader) decides when to consume an asset. Owned assets
...
1. **All participants were online at check time.** The queue verifies
   that every participant in the asset's set has an active, bidirectional
   P2P connection and is within a few blocks of the local indexer height.
   This check is at most ~1 second old.
...
3. **No real-time liveness guarantee.** A participant can go offline
...
4. **Followers trust the leader's choice.** `take_unowned(id)` performs
   a plain database lookup with no liveness check. Followers assume the
   leader picked a valid asset.

In summary, the queue provides a **best-effort liveness filter**: it
avoids obviously-stale assets but does not guarantee that the protocol
will succeed. Protocol failures due to participant churn are expected
and handled by retrying with a fresh asset.
...
- **Offline assets** — assets where at least one participant has lost
  its P2P connection — are the only ones subject to discarding. They are
...
Both owned and unowned assets are persisted to RocksDB. Owned assets are
additionally kept in an in-memory queue (the `DoubleQueue` described
below) so the leader can quickly select the next usable one. Unowned
assets live only in RocksDB, indexed by ID, and are retrieved on demand
when the leader broadcasts which asset to use.

Followers do not run their own generation loops. Instead, the generic
protocol runner (`crates/node/src/protocol.rs`) handles incoming
