...
Every asset is generated by a set of participants. The participant who
initiates the computation is the **leader**, while the others are
**followers**. All participants hold a different share of the same
asset, identified by a common unique ID.

The leader of the computation becomes the **owner** of the resulting
asset, while the followers become **borrowers**. Owned assets sit in a
queue and are consumed in roughly FIFO order. Unowned assets are only
consumed by borrowers when the owner requests it.
...
throughput. Borrowers are passive.
...
**online** if all of its borrowers are online from the perspective of
the asset owner, and **offline** otherwise.
...
3. **Borrowers trust the owner's choice.** `take_unowned(id)` performs
   a plain database lookup with no liveness check. Borrowers assume the
   owner picked a valid asset.
