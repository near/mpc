...
# A few reasonable ranges for testing
TIMEOUT_VALUES = [2, 5, 10]
INTERVAL_VALUES = [0.1, 0.5, 1.0]
ATTEMPT_VALUES = [3, 5]
    ...
    for timeout in TIMEOUT_VALUES:
        for interval in INTERVAL_VALUES:
            for attempts in ATTEMPT_VALUES:
                ok = validate_image_hash(
                    TEST_DIGEST,
                    DSTACK_CONFIG,
                    rpc_request_timeout_secs=timeout,
                    rpc_request_interval_secs=interval,
                    rpc_max_attempts=attempts,
                )

                if not ok:
                    failures.append(
                        f"Failed for timeout={timeout}, interval={interval}, attempts={attempts}"
                    )

    assert not failures, (
        "validate_image_hash() failed for some parameter permutations:\n"
        + "\n".join(failures)
    )
