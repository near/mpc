...
use near_synth_bm::account::{accounts_from_dir, update_account_nonces, Account};
use near_synth_bm::block_service::BlockService;
use near_synth_bm::rpc::{ResponseCheckSeverity, RpcResponseHandler};
...
enum Commands {
    ...
    // TODO(#242): add a sub-command to help set up accounts and keys
}
...
pub async fn benchmark_mpc_sign_impl(
    ...
    for i in 0..args.num_transactions {
        ...
        let transaction = SignedTransaction::call(
            ...
            args.deposit,
            "sign".to_string(),
            new_random_mpc_sign_args(&mut rng, args.key_version)
                .to_string()
                .into_bytes(),
            args.gas,
        ...
        if i > 0 && i % 200 == 0 {
            info!(
                "sent {i} transactions in {:.2} seconds",
    ...
    info!(
        "Received all RPC responses after {:.2} seconds",
        timer.elapsed().as_secs_f64()
    );
