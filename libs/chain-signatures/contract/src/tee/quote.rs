...
pub fn verify_mocked_quote() -> (VerifiedReport, Value) {
    let tcb_info = json!(
        {
    ...
    let collateral = get_collateral(raw_quote_collateral).unwrap();
    ...
    let verified_report = verify::verify(&quote, &collateral, now).unwrap();

    (verified_report, tcb_info)
}

#[cfg(test)]
mod tests {
    use super::{replay_app_compose, replay_rtmr, verify_codehash};
    use crate::tee::quote::verify_mocked_quote;
    use hex::encode;

    #[test]
    fn test_verify_quote_and_codehash() {
        let (verified_report, tcb_info) = verify_mocked_quote();
        let event_log = tcb_info["event_log"].as_array().unwrap();
        ...
        let rtmr3 = encode(verified_report.report.as_td10().unwrap().rt_mr3);
