...
/// Reasons a `ChainEntry` fails validation. `NonEmptyBTreeMap` already enforces
/// non-empty + unique-`ProviderId` at borsh-deserialize time, so those cases are
/// absent here.
...
impl ChainEntry {
    /// Validate shape invariants. See [`ChainEntryValidationError`] for the rules.
    pub fn validate(&self) -> Result<(), ChainEntryValidationError> {
        if self.quorum == 0 {
            return Err(ChainEntryValidationError::ZeroQuorum);
        }
        let providers_len = u64::try_from(self.providers.len()).map_err(|e| {
            ChainEntryValidationError::ProvidersLenOverflow {
                len: self.providers.len(),
                reason: e.to_string(),
            }
        })?;
        if self.quorum > providers_len {
            return Err(ChainEntryValidationError::QuorumExceedsProviders {
                quorum: self.quorum,
                providers_len,
            });
        }
        for (id, config) in self.providers.iter() {
