...
impl ChainEntry {
    /// Checks the rules the contract enforces before a proposal can apply; see [`ChainEntryValidationError`].
    pub fn validate(&self) -> Result<(), ChainEntryValidationError> {
        let ChainEntry { providers, quorum } = self;
        if *quorum == 0 {
...
mod tests {
    ...
    fn validate_chain_entry__should_accept_well_formed_entry() {
        ...
        // When / Then
        entry.validate().expect("well-formed entry should validate");
    }
}
