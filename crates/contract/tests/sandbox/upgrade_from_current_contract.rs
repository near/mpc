...
async fn test_propose_contract_max_size_upload() {
    ...
    // A single-tx code blob is bounded by the RPC payload limit (~1.5 MiB).
...
/// Exercises start → multiple chunk uploads → finalize → vote → deploy, and
/// asserts the deployed binary's hash matches what was uploaded.
#[tokio::test]
async fn test_chunked_upload_multi_chunk_and_deploy() {
    ...
    assert!(
        chunks_uploaded >= 3,
        "should have uploaded at least 3 chunks"
    );
...
/// `start_contract_upload` from a non-voter must be rejected.
#[tokio::test]
...
/// Starting a second upload without clearing the first must be rejected.
#[tokio::test]
...
/// A chunk that overruns the declared `total_size` must be rejected.
#[tokio::test]
...
/// Finalizing before the full payload has been uploaded must be rejected.
#[tokio::test]
...
/// After `clear_staged_contract`, the caller can start a fresh upload.
#[tokio::test]
...
/// Uploads and deploys the ~2 MiB `test-large-contract` so the test fails if
/// chunked upload can't handle a binary above the RPC's single-tx limit.
#[tokio::test]
