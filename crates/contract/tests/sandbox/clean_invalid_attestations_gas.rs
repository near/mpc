...
use near_workspaces::{Account, Contract, types::Gas};
use serde_json::json;
...
/// The budget that shipped before this was raised. Retained by the measurement tools to
...
/// Removals the budget is sized to cover within a full scan. Measured capacity is ~7; this
/// sits below it so ordinary measurement variance cannot flip the guard. The worst case —
/// every scanned entry removable — is deliberately not funded.
const REMOVALS_COVERED: u32 = 6;
/// Sandbox understates production, so guards scale measured cost by this factor before
/// comparing against the budget.
///
/// Calibrated against two zero-removal sweeps on `v1.signer-prod.testnet` — `max_scan: 1`
/// burned 3.709 TGas and `max_scan: 100` (28 entries) burned 9.503 TGas — which separate the
/// two terms: 3.49 TGas fixed and 0.215 TGas per entry scanned, against 2.60 and 0.177 here.
/// That is x1.34 on the fixed cost and x1.21 per entry, so a single x1.35 over-estimates the
/// per-entry term and errs toward funding more than needed.
///
/// Participant count is not a factor: it costs the ordinary per-entry rate, measured identical
/// at 10 and 17 participants. The gap is a deployment carrying more contract state to
/// deserialize per call and larger real attestation payloads to read per entry.
///
/// The per-removal cost is still sandbox-only — both production sweeps removed nothing.
...
/// have to be sized together; this pins them so they cannot drift apart again. The
...
async fn clean_invalid_attestations__budget_covers_deployed_map_sizes() {
    ...
    for (label, total, expired) in [("mainnet", 14u32, 1u32), ("testnet", 33, 10)] {
        ...
        seed_attestations_from(&contract, seeded - expired, expired, true).await;
...
async fn clean_invalid_attestations__observable_effects_when_budget_exhausted() {
    ...
    let result = caller
        ...
        .gas(Gas::from_tgas(PRE_FIX_BUDGET_TGAS))
        .transact()
        .await
        .unwrap();
...
///
/// Gas is always set explicitly: `near-workspaces`' `max_gas()` attaches 1000 TGas, above
/// NEAR's 300 TGas prepaid ceiling, so it cannot model what the chain will actually allow.
async fn try_sweep_receipt_tgas(
    ...
    let result = caller
        ...
        .gas(Gas::from_tgas(prepaid_tgas as u64))
        .transact()
        .await
        .unwrap();
...
async fn seed_attestations(contract: &Contract, count: u32, expired: bool) {
    seed_attestations_from(contract, 0, count, expired).await
}

/// Seeds `count` entries with keys starting at `first_key`, so a caller can lay down a run of
/// surviving entries and a run of removable ones without their keys colliding.
async fn seed_attestations_from(contract: &Contract, first_key: u32, count: u32, expired: bool) {
    let mut seeded = 0;
    while seeded < count {
        let chunk = SEED_CHUNK.min(count - seeded);
        let result = contract
            .as_account()
            .call(contract.id(), "bench_seed_attestations")
            .args_json(json!({
                "offset": first_key + seeded,
                "count": chunk,
                "expired": expired,
            }))
            .gas(Gas::from_tgas(MAX_PREPAID_TGAS as u64))
            .transact()
            .await
            .unwrap();
        assert!(
            result.is_success(),
            "seeding failed at key {}: {:?}",
            first_key + seeded,
            result.failures()
        );
        seeded += chunk;
