...
use crate::sandbox::{
    common::{
        ...
        generate_participant_and_submit_attestation, get_state, init_env, IntoContractType,
        ALL_SIGNATURE_SCHEMES, GAS_FOR_VOTE_CANCEL_KEYGEN, PARTICIPANT_LEN,
    },
...
async fn test_keygen() -> anyhow::Result<()> {
    ...
    let public_key: dtos::PublicKey = "ed25519:J75xXmF7WUPS3xCm3hy2tgwLCKdYM1iJd4BWF8sWVnae"
        .parse()
        .unwrap();
    vote_public_key(&contract, &accounts, key_event_id, public_key.clone())
        .await
        .unwrap();
...
async fn setup_resharing_state(
    ...
    let (worker, contract, persistent_participants, _) =
