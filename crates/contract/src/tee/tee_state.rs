use crate::{
    primitives::{
        key_state::AuthenticatedParticipantId, participants::Participants, votes::VoterSet,
    },
...
impl TeeState {
    ...
    pub fn vote(
        ...
        self.votes.vote(participant, code_hash.into())
    }
    ...
    pub fn vote_launcher(
        ...
        self.launcher_votes.vote(participant.clone(), action.into())
    }
    ...
    pub fn vote_measurement(
        ...
        self.measurement_votes.vote(participant, action.into())
    }
...
mod tests {
    ...
    use crate::primitives::votes::PROPOSAL_HASH_BYTES;
    ...
    fn test_clean_non_participants_removes_stale_votes() {
        ...
        for account_id in &account_ids[0..2] {
            ...
            tee_state.votes.vote(auth_id, malicious_hash.into());
        }
        assert_eq!(tee_state.votes.all().pop_first().unwrap().1.len(), 2);
        ...
        assert_eq!(tee_state.votes.all().len(), 0);
        ...
        let vote_count = tee_state.votes.vote(auth_id.clone(), malicious_hash.into());
        assert_eq!(
    ...
    fn test_clean_non_participants_removes_stale_launcher_and_measurement_votes() {
        ...
        tee_state
            .launcher_votes
            .vote(auth_id.clone(), launcher_action.clone().into());

        let state = tee_state.launcher_votes.all();
        ...
        assert_eq!(
            state,
            BTreeMap::from([(expected_hash, BTreeSet::from([auth_id]))])
        );
