...
impl TeeState {
    ...
    /// stored attestation for `tls_public_key`. This is the "refresh on use" signal that
    /// keeps an in-use launcher hash from expiring. Callers MUST only invoke this for a
    /// current participant's attestation — usage by a non-participant (e.g. a prospective
    /// node) must not keep a launcher hash alive.
    pub(crate) fn refresh_launcher_usage(&mut self, tls_public_key: &Ed25519PublicKey) {
...
mod tests {
    ...
    fn refresh_launcher_usage__keeps_attested_launcher_alive() {
        ...
        tee_state
            .add_participant(node_id.clone(), attestation, Duration::from_secs(0), TTL)
            .unwrap();
        ...
        tee_state.refresh_launcher_usage(&node_id.tls_public_key);
        ...
        tee_state.refresh_launcher_usage(&bogus_ed25519_public_key());
    }
