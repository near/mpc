...
mod tests {
    ...
    use crate::primitives::{
        ...
        test_utils::{bogus_ed25519_near_public_key, bogus_ed25519_public_key, gen_participants},
    };
    use crate::tee::test_utils::set_block_timestamp;
    use assert_matches::assert_matches;
    use mpc_attestation::attestation::{Attestation, MockAttestation, VerificationError};
    ...
    fn mock_valid_attestation(node_id: NodeId) -> NodeAttestation {
        NodeAttestation {
            node_id,
    ...
    /// Sets [`AccountId`] as the signer and authenticates it against [`Participants`].
    fn authenticate_as(
    ...
    fn clean_invalid_attestations__should_remove_expired_entries() {
        ...
        let fresh_account: AccountId = "fresh.near".parse().unwrap();
        let stale_account: AccountId = "stale.near".parse().unwrap();
        let fresh_node = node_id_for(&fresh_account);
        let stale_node = node_id_for(&stale_account);

        let fresh = mock_attestation_with_expiry(FRESH_EXPIRY_SECONDS);
        let stale = mock_attestation_with_expiry(STALE_EXPIRY_SECONDS);
    ...
    fn clean_invalid_attestations__should_honor_max_scan() {
        ...
        let expired = mock_attestation_with_expiry(EXPIRY_SECONDS);

        for idx in 0..10 {
            let account_id: AccountId = format!("node{idx}.near").parse().unwrap();
            let node_id = node_id_for(&account_id);
            tee_state
    ...
    fn clean_invalid_attestations__should_keep_valid_entries() {
        ...
        let node_id = node_id_for(&account_id);
        let attestation = mock_attestation_with_expiry(FUTURE_EXPIRY_SECONDS);
        tee_state
    ...
    fn updating_existing_participant_returns_existing_participant() {
        ...
        assert_eq!(
            ...
            Ok(ParticipantInsertion::UpdatedExistingParticipant(
                mock_valid_attestation(participant_id)
            ))
        );
    }
    ...
    fn verify_and_store_mock__should_preserve_node_id_integrity() {
        ...
        // then
        assert_eq!(
            tee_state.stored_attestations.get(&node_id.tls_public_key),
            Some(&mock_valid_attestation(node_id))
        );
    }
    ...
    fn internal_storage_distinguishes_participants_by_tls_key() {
        ...
        let node_1 = node_id_for(&alice);
        let node_2 = node_id_for(&bob);
    ...
    fn re_verify_validates_fresh_attestation() {
        // given
        let mut tee_state = TeeState::default();
        let account_id: AccountId = "fresh.near".parse().unwrap();
        let node_id = node_id_for(&account_id);
        ...
        let attestation = mock_attestation_with_expiry(NOW_SECONDS);
    ...
    fn test_re_verify_rejects_expired_attestation() {
        // given
        let mut tee_state = TeeState::default();
        let account_id: AccountId = "about_to_be_expired.near".parse().unwrap();
        let node_id = node_id_for(&account_id);
        ...
        let attestation = mock_attestation_with_expiry(EXPIRY_TIMESTAMP_SECONDS);
        ...
        // then
        assert_matches!(status, TeeQuoteStatus::Invalid(msg) if msg.contains("has expired"));
    }
    ...
    fn re_verify_succeeds_within_expiry_time() {
        // given
        let mut tee_state = TeeState::default();
        let account_id: AccountId = "valid_check.near".parse().unwrap();
        let node_id = node_id_for(&account_id);
    ...
    fn test_re_verify_returns_invalid_for_missing_node() {
        ...
        let account_id: AccountId = "ghost.near".parse().unwrap();
        let node_id = node_id_for(&account_id);
    ...
    fn validate_tee_returns_partial_when_participant_has_no_attestation() {
        ...
        store_valid_attestations(&mut tee_state, &participant_list[..2], tee_upgrade_duration);
    ...
    fn validate_tee_returns_partial_when_attestation_is_expired() {
        ...
        let node_id = create_node_id(account_id, &participant_info.tls_public_key);
        let expiring_attestation = mock_attestation_with_expiry(expiry_time_secs);
        tee_state
    ...
    fn validate_tee_returns_full_when_attestation_not_yet_expired() {
        ...
        for (i, (account_id, _, participant_info)) in participant_list.iter().enumerate() {
            ...
            let attestation = if i == 2 {
                mock_attestation_with_expiry(expiry_time_secs)
            } else {
                MockAttestation::Valid
            };
            tee_state
    ...
    fn verify_and_store_mock__should_reject_tls_key_owned_by_other_account() {
        ...
        let alice: AccountId = "alice.near".parse().unwrap();
        let alice_node = create_node_id(&alice, &tls_public_key);
        tee_state
            .verify_and_store_mock(
                alice_node.clone(),
                MockAttestation::Valid,
        ...
        let attacker_node = create_node_id(&attacker, &tls_public_key);
        ...
        assert_eq!(
            tee_state.stored_attestations.get(&tls_public_key),
            Some(&mock_valid_attestation(alice_node))
        );
    }
    ...
    fn verify_and_store_mock__should_allow_same_account_to_update_its_own_entry() {
        ...
        let alice: AccountId = "alice.near".parse().unwrap();
        let initial_node = create_node_id(&alice, &tls_public_key);
        tee_state
        ...
        let rotated_node = create_node_id(&alice, &tls_public_key);
        let result = tee_state.verify_and_store_mock(
            rotated_node.clone(),
            MockAttestation::Valid,
            TEE_UPGRADE_DURATION,
        ...
        assert_eq!(
            tee_state.stored_attestations.get(&tls_public_key),
            Some(&mock_valid_attestation(rotated_node))
        );
    }
    ...
    fn revert_dstack_store__should_restore_the_displaced_entry_on_update() {
        ...
        let original_entry = mock_valid_attestation(original_node);
        let updated_entry = mock_valid_attestation(updated_node);
        assert_eq!(
            insertion,
            ParticipantInsertion::UpdatedExistingParticipant(original_entry.clone())
        );
        assert_eq!(
            tee_state.stored_attestations.get(&tls_public_key),
            Some(&updated_entry)
    ...
    fn verify_and_store_mock__should_reject_invalid_attestations() {
        ...
        assert_matches!(
            ...
            Err(AttestationSubmissionError::InvalidAttestation(
                VerificationError::InvalidMockAttestation
            ))
        )
    }
    ...
    fn verify_and_store_dstack__should_reject_and_store_nothing_when_post_dcap_checks_fail() {
        ...
        assert_matches!(
            ...
            )) if msg.contains("allowed mpc image hashes list is empty")
        );
    ...
    fn test_clean_non_participant_votes_removes_stale_votes() {
        ...
        let all_participants = gen_participants(5);
        let account_ids: Vec<AccountId> = all_participants
            .participants()
            .iter()
    ...
    fn test_clean_non_participants_removes_stale_launcher_and_measurement_votes() {
        let all_participants = gen_participants(3);
        let account_ids: Vec<AccountId> = all_participants
            .participants()
            .iter()
            .map(|(account_id, _, _)| account_id.clone())
