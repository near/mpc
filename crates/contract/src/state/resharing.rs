...
// Membership during a resharing is ambiguous between the current and prospective
// sets, so `IsParticipant` is deliberately unimplemented; use
// `is_participant_or_prospective_participant` or ask the underlying states.
static_assertions::assert_not_impl_any!(ResharingContractState: IsParticipant<near_sdk::AccountId>);
