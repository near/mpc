...
impl MpcContract {
    ...
    /// Whether an in-flight `Dstack` attestation verification is recorded for
    /// `account_id`. Lets the async-attestation sandbox tests assert that the
    /// pending entry was cleaned up (on rejection, timeout, or failed charge)
    /// rather than leaked — which the production API can't observe.
