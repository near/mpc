...
pub(super) async fn setup_localnet(validator_home: PathBuf, observer_home: PathBuf) -> Localnet {
...
impl LocalNode {
    ...
    fn read_node_public_key(&self) -> String {
...
struct LocalNodeBuilder {
    home_dir: PathBuf,
    ports: Option<PortsConfig>,
}

impl LocalNodeBuilder {
    async fn start(self) -> LocalNode {
    ...
    fn new(node_name: &str, home_dir: PathBuf) -> Self {
    ...
    fn with_genesis_from(self, source: &LocalNode) -> Self {
        std::fs::copy(
            source.home_dir.join("genesis.json"),
    ...
    fn with_boot_nodes(self, boot_node: &str) -> Self {
        let mut config = self.config();
    ...
    fn without_validator_key(self) -> Self {
        let validator_key_path = self.home_dir.join("validator_key.json");
    ...
    fn with_consensus_min_peers(self, min_peers: u64) -> Self {
        let mut config = self.config();
    ...
    fn with_contract(self, contract: Contract, wasm: &[u8]) -> Self {
        let genesis_path = self.home_dir.join("genesis.json");
