...
impl ProtocolContractState {
    /// Returns the current epoch for any initialized state.
    ...
    /// Returns the next domain ID for any initialized state.
    ...
    /// Returns the domain registry for Running or Resharing states.
    ...
    /// Returns a domain config by domain ID, if found.
    ...
    pub fn threshold(&self) -> Option<Threshold> {
        match self {
            ProtocolContractState::Initializing(state) => {
                Some(state.generating_key.parameters.threshold)
            }
            ProtocolContractState::Running(state) => Some(state.parameters.threshold),
            ProtocolContractState::Resharing(state) => {
                Some(state.previous_running_state.parameters.threshold)
            }
    ...
    /// Finds the public key for a domain in Running or Resharing state.
