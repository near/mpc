...
/// What to do to A0 between the forward and back directions of a
/// round-trip back-migration test.
enum BetweenDirections {
    /// A0 stays running. Exercises the in-memory state-transition path
    /// between forward and back directions; doesn't model the
    /// disk-persisted-restart scenario.
    None,
    /// A0 is killed and restarted from its on-disk home dir. Models the
    /// "operator decommissioned A then resurrected from disk" scenario
    /// investigated in #2121. Asserts that keyshares survive the restart.
    KillAndRestartA,
}

/// Shared scaffolding for the back-migration round-trip tests. Sets up
/// the cluster, runs forward A→B, applies `between` to A0, runs back
/// B→A, kills B0, and asserts sign+ckd work with A0+A1.
///
/// Per direction the migration round does:
///   1. Register backup service
///   2. GET keyshares from source node
///   3. Initiate node migration
///   4. PUT keyshares to target node
...
/// neither variant can exercise the on-chain stale-attestation failure
/// mode — the leading suspect for #2121's production symptom. These
/// are positive-baseline regression tests for the back-migration code
/// path, not a reproducer for #2121.
async fn run_back_migration_round_trip(port_seed: u16, between: BetweenDirections) {
    let backup_cli = must_get_backup_cli_path();

    // Given: 2 participants (A0, A1) + 1 migration target (B0). A1 stays a
    ...
    // Hook: apply the variant-specific between-directions action.
    match between {
        BetweenDirections::None => {}
        BetweenDirections::KillAndRestartA => {
            // Simulate operator decommissioning the old node and bringing
            // it back online later for the back-migration. `kill_nodes` +
            // `start_nodes` preserves the node's home dir (keyshares stay
            // on disk) — that disk state is the trigger for the suspected
            // P1 early-return.
            //
            // Snapshot A0's keyshare-file listing before kill so we can
            // verify it's still there after start. Keyshares live in
            // `home_dir/permanent_keys/` (see
            ...
            // Snapshot A0's indexer height before the kill so the
            // post-restart readiness check (below) can prove the indexer
            // resumed and advanced past where it was — not just bound the
            // web port.
            ...
            // Validate the assumption the test rests on: A0's keyshares
            // are still on disk after the restart. If they're missing or
            // different, the test no longer models production's
            // back-migration scenario (e.g. someone swapped `start_nodes`
            // for `reset_and_start_nodes`, which wipes the home dir).
    ...
    // Kill B0 before the final assertions so the sign + ckd requests
    // cannot be served by the now-demoted node — that proves A0 + A1 are
    // carrying the workload alone.
    cluster
        .kill_nodes(&[b_idx])
...
/// Back-migration A→B→A without kill+restart of A0. Stable variant that
/// exercises the back-migration code path through A0's in-memory state
/// transitions; doesn't depend on A0 surviving a sandbox-blocks-during-
/// catch-up window (see #3366).
///
/// The kill+restart sibling `_a_to_b_to_a` owns coverage of the
/// disk-persisted-restart scenario from #2121.
#[tokio::test]
#[expect(non_snake_case)]
...
/// Back-migration A→B→A with kill+restart of A0 between the two
/// directions, modeling the disk-persisted-state scenario investigated
/// in #2121.
#[tokio::test]
#[expect(non_snake_case)]
