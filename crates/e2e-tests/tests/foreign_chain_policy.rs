...
async fn foreign_chain_policy__should_require_unanimity_for_auto_voting() {
    ...
    let partial_votes_timeout = Duration::from_secs(60);
    (|| async {
    ...
    .retry(
        ConstantBuilder::default()
            ...
            .with_max_times(
                (partial_votes_timeout.as_millis() / common::POLL_INTERVAL.as_millis()) as usize,
            ),
    )
    .await
    .expect("timed out waiting for 2 partial votes");

    // then — verify exactly 2 votes and no policy
    let proposals = cluster
        .view_foreign_chain_policy_proposals()
        .await
        .expect("failed to view proposals");
    assert_eq!(
        proposals.proposal_by_account.len(),
        2,
        "expected exactly two votes before unanimity"
    );
    ...
    let policy_timeout = Duration::from_secs(30);
    (|| async {
        let proposals = cluster
            .view_foreign_chain_policy_proposals()
            .await
