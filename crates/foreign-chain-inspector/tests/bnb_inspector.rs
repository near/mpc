...
async fn extract_returns_correct_value_when_finalized(
    ...
    #[values(BnbExtractor::Log { log_index: 1 }, BnbExtractor::BlockHash)]
    extractor: BnbExtractor,
) {
    // given
    let tx_id = BnbTransactionHash::from([3; 32]);
    ...
    let mock_client = mock_bnb_client(block_response, tx_response);
    let inspector = BnbInspector::new(mock_client);
...
async fn extract_succeeds_when_finality_block_equals_tx_block() {
    // given
    let tx_id = BnbTransactionHash::from([3; 32]);
    let expected_block_hash = BnbBlockHash::from([4; 32]);
    ...
    let extracted_values = inspector
        .extract(
            ...
            vec![BnbExtractor::BlockHash],
        )
        .await
        .unwrap();

    // then
    let expected_extractions = vec![BnbExtractedValue::BlockHash(expected_block_hash)];
...
async fn extract_returns_error_when_not_finalized() {
    // given
    let tx_id = BnbTransactionHash::from([1; 32]);
...
async fn extract_returns_empty_when_no_extractors_provided() {
    // given
    let tx_id = BnbTransactionHash::from([11; 32]);
    ...
    // then
    let expected_extractions: Vec<BnbExtractedValue> = vec![];
    assert_eq!(expected_extractions, extracted_values);
}
...
async fn extract_propagates_rpc_client_errors() {
    // given
    let tx_id = BnbTransactionHash::from([9; 32]);
...
async fn inspector_extracts_block_hash_via_http_rpc_client() {
    ...
    let tx_id = BnbTransactionHash::from([9; 32]);
    let expected_block_hash = BnbBlockHash::from([5; 32]);
...
async fn extract_returns_error_when_log_index_out_of_bounds() {
    ...
    let response = inspector
        .extract(
            tx_id,
            EthereumFinality::Finalized,
            vec![BnbExtractor::Log { log_index: 5 }],
        )
        .await;
...
async fn extract_returns_correct_log_by_evm_log_index() {
    ...
    let extracted_values = inspector
        .extract(
            tx_id,
            EthereumFinality::Finalized,
            vec![BnbExtractor::Log { log_index: 21 }],
        )
        .await
        .unwrap();

    // then
    let expected_extractions = vec![BnbExtractedValue::Log(expected_log)];
    assert_eq!(expected_extractions, extracted_values);
}
...
fn expected_extracted_value(
    ...
    match extractor {
        BnbExtractor::BlockHash => {
            BnbExtractedValue::BlockHash(From::from(*tx_response.block_hash.as_fixed_bytes()))
        }
        BnbExtractor::Log { log_index } => {
            ...
            BnbExtractedValue::Log(log.clone())
        }
    }
}
