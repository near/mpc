...
pub(super) struct PerDomainData {
    ...
    /// domain's keygen/presign/sign — see #3164.
...
impl EcdsaSignatureProvider {
    ...
    pub fn new(
        ...
        // cait-sith triple generation always runs with exactly `t` parties, so
        // the full set of `t`s this node must serve is the set of distinct
        // per-domain reconstruction thresholds, known up front (no on-demand
        // creation needed). The contract enforces that all CaitSith domains
        // share one `t`, so this is typically a single store.
