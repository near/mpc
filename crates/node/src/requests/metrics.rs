...
pub static MPC_PENDING_REQUEST_QUEUE_SIZE: LazyLock<prometheus::IntGauge> = LazyLock::new(|| {
    prometheus::register_int_gauge!(
        "mpc_pending_request_queue_size",
        "Number of pending requests in the queue"
    )
    .unwrap()
});

pub static MPC_PENDING_REQUEST_QUEUE_ATTEMPTS_GENERATED: LazyLock<prometheus::IntCounter> =
    LazyLock::new(|| {
        prometheus::register_int_counter!(
            "mpc_pending_request_queue_attempts_generated",
            "Number of attempts generated by the pending queue to compute a response as the leader"
        )
        .unwrap()
    });
...
pub static MPC_PENDING_VERIFY_FOREIGN_TX_QUEUE_SIZE_TOTAL: LazyLock<prometheus::IntGauge> =
    LazyLock::new(|| {
        prometheus::register_int_gauge!(
            "mpc_pending_verify_foreign_txs_queue_size_total",
            "Number of pending verify foreign tx requests in the queue"
        )
        .unwrap()
    });
pub static MPC_PENDING_VERIFY_FOREIGN_TX_QUEUE_ATTEMPTS_GENERATED_TOTAL: LazyLock<
    prometheus::IntCounter,
> = LazyLock::new(|| {
    prometheus::register_int_counter!(
            "mpc_pending_verify_foreign_txs_queue_attempts_generated_total",
...
> = LazyLock::new(|| {
    prometheus::register_int_counter!(
        "mpc_pending_verify_foreign_txs_queue_blocks_indexed_total",
        "Number of blocks indexed by the pending verify foreign tx queue"
    )
    .unwrap()
});
...
> = LazyLock::new(|| {
    prometheus::register_int_counter!(
        "mpc_pending_verify_foreign_txs_queue_finalized_blocks_indexed_total",
        "Number of finalized blocks indexed by the pending verify foreign tx queue"
    )
    .unwrap()
});
pub static MPC_PENDING_VERIFY_FOREIGN_TXS_QUEUE_REQUESTS_INDEXED_TOTAL: LazyLock<
    prometheus::IntCounter,
> = LazyLock::new(|| {
    prometheus::register_int_counter!(
        "mpc_pending_verify_foreign_txs_queue_requests_indexed_total",
        "Number of verify foreign tx requests indexed by the pending verify foreign tx queue"
    )
    .unwrap()
});
pub static MPC_PENDING_VERIFY_FOREIGN_TXS_QUEUE_RESPONSES_INDEXED_TOTAL: LazyLock<
    prometheus::IntCounter,
> = LazyLock::new(|| {
    prometheus::register_int_counter!(
...
pub static MPC_PENDING_VERIFY_FOREIGN_TXS_QUEUE_MATCHING_RESPONSES_INDEXED_TOTAL: LazyLock<
    prometheus::IntCounter,
> = LazyLock::new(|| {
    prometheus::register_int_counter!(
...
pub static VERIFY_FOREIGN_TXS_REQUEST_RESPONSE_LATENCY_BLOCKS: LazyLock<prometheus::Histogram> =
    LazyLock::new(|| {
        ...
        prometheus::register_histogram!(
            "mpc_verify_foreign_txs_request_response_latency_blocks",
            "The number of blocks between when a verify foreign tx request is seen and when the corresponding response is seen.",
            buckets).
            unwrap()
    });
pub static VERIFY_FOREIGN_TXS_REQUEST_RESPONSE_LATENCY_SECONDS: LazyLock<prometheus::Histogram> =
    LazyLock::new(|| {
        prometheus::register_histogram!(
            "mpc_verify_foreign_txs_request_response_latency_seconds",
            "The duration, in seconds, between when a verify foreign tx request is seen and when the corresponding response is seen.",
            // 2s - 3s - 4.5s .... - 115s
