...
pub fn convert_participant_infos(
    ...
    for (account_id, id, info) in &threshold_parameters.participants.participants {
        let (address, port) = match info.url.parse::<NodeAddress>() {
            Ok(node_address) => (
                node_address.host(),
                port_override.unwrap_or(node_address.port()),
            ),
            Err(error) => {
                tracing::warn!(
                    target: "mpc",
...
mod tests {
    ...
    /// A scheme-less `host:port` must be accepted.
    #[test]
    fn convert_participant_infos__should_accept_scheme_less_url() {
        // Given
        let mut entries = create_chain_participant_infos().participants;
        entries[0].2.url = "10.101.0.122:3000".to_string();
        ...
        let entry = converted
            ...
            .unwrap();
        assert_eq!(entry.address, "10.101.0.122");
