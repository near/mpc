...
// --- STABLE METRICS ---
static TOKIO_RUNTIME_TOTAL_BUSY_DURATION_SECS: LazyLock<IntGauge> = LazyLock::new(|| {
    register_int_gauge!(
        "tokio_runtime_total_busy_duration_secseconds",
        "Total time worker threads were busy since runtime start (ms)."
    )
    .unwrap()
});
...
pub(crate) async fn monitor_runtime_metrics(runtime_monitor: RuntimeMonitor) {
    for runtime_metrics in runtime_monitor.intervals() {
        TOKIO_RUNTIME_TOTAL_BUSY_DURATION_SECS
        ...
        {
            TOKIO_RUNTIME_MEAN_POLL_DURATION_SECONDS
                .set(runtime_metrics.mean_poll_duration.as_secs() as f64);
