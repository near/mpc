...
use crate::indexer::{tx_sender, ForeignChainPolicyReader, IndexerAPI};
    ...
    TransactionSender: tx_sender::TransactionSender + 'static,
{
    ...
    async fn run_mpc(
        ...
        chain_txn_sender: TransactionSender,
        foreign_chain_policy_reader: Arc<dyn ForeignChainPolicyReader + Send + Sync>,
    ...
    async fn maybe_vote_foreign_chain_policy(
        ...
        let Some(local_policy) = config_file.foreign_chains.to_policy() else {
            tracing::info!("foreign_chains config is empty; skipping foreign chain policy auto-vote");
            return Ok(());
        };
        ...
        chain_txn_sender
            .send(ChainSendTransactionRequest::VoteForeignChainPolicy(
                ChainVoteForeignChainPolicyArgs { policy: local_policy },
            ))
            .await
