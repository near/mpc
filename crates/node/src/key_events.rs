...
pub struct ResharingArgs {
    ...
    /// `old_participants.threshold` per-key so the underlying resharing
    /// protocol sees the right `t` on each side of the transition.
...
async fn resharing_computation_inner(
    ...
    // domain config; the old one is the previous epoch's per-domain `t`. We
    // patch `old_participants.threshold` per-key so the underlying resharing
    // protocol sees the correct `t` on each side of the transition. For
    // robust-ECDSA the reconstruction lower bound equals `t`, so no
    // protocol-specific translation is needed here.
    let new_threshold =
        ReconstructionLowerBound::from(usize::try_from(domain.reconstruction_threshold.inner())?);
...
mod tests {
    ...
    fn make_test_resharing_args() -> Arc<ResharingArgs> {
        Arc::new(ResharingArgs {
            ...
            old_reconstruction_thresholds: HashMap::new(),
