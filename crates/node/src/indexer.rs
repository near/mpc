use crate::{
    indexer::{
        ...
        types::{
            ChainCKDRequest, ChainGetPendingCKDRequestArgs,
            ChainGetPendingVerifyForeignTxRequestArgs, ChainGetPendingSignatureRequestArgs,
            ChainSignatureRequest, ChainVerifyForeignTxRequest, GetAttestationArgs,
        },
    },
...
impl IndexerViewClient {
    ...
    pub(crate) async fn get_pending_verify_foreign_tx_request(
        &self,
        mpc_contract_id: &AccountId,
        chain_verify_foreign_tx_request: &ChainVerifyForeignTxRequest,
    ) -> anyhow::Result<Option<YieldIndex>> {
        let get_pending_request_args: Vec<u8> =
