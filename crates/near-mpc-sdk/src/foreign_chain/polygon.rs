...
pub use crate::foreign_chain::evm::EvmBlockHash as PolygonBlockHash;
...
impl EvmChainVariant for Polygon {
    fn wrap(request: EvmRpcRequest) -> ForeignChainRpcRequest {
        ForeignChainRpcRequest::Polygon(request)
    }
}

pub type PolygonRequest<TxId, Finality> = EvmRequest<Polygon, TxId, Finality>;

impl ForeignChainRequestBuilder<PolygonRequest<NotSet, NotSet>, NotSet> {
...
mod test {
    ...
    #[test]
    fn build_wraps_into_polygon_variant() {
        // given / when
        let (_verifier, request_args) = ForeignChainRequestBuilder::new_polygon()
        ...
        // then
        assert_matches!(request_args.request, ForeignChainRpcRequest::Polygon(_));
    }
}
