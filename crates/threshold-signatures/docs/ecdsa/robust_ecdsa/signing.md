...
$$
\mathsf{Com}(a_{ji}; \rho_{ji}) = \sum_{m=0}^{t} i^m \cdot A_{j,m}\\
\mathsf{Com}(b_{ji}; \sigma_{ji}) = \sum_{m=1}^{2t} i^m \cdot B_{j,m}
$$
...
5. Each $P_i$ computes $R_i \gets k_i \cdot G$
...
$\forall j \in \\{t+2.. n\\},\quad \mathsf{Interpolation}(R_1, \ldots R_{t+1}; j) =  R_j$
...
7. Each $P_i$ computes $\pi_i \gets \mathsf{Prove}\big(w_i \cdot G, \mathsf{Com}_a(i), \mathsf{Com}_b(i), R_i;\ a_i, b_i, \rho_i, \sigma_i\big)$ where $\mathsf{Com}_a(j) = \sum_l \sum_{m=0}^{t} j^m \cdot A_{l,m}$ and $\mathsf{Com}_b(j) = \sum_l \sum_{m=1}^{2t} j^m \cdot B_{l,m}$
8. $\star$ Each $P_i$ sends $\pi_i$ to every party.

**Round 4:**

1. $\bullet$ Each $P_i$ waits to receive $\pi_j$ from every party.
2. $\blacktriangle$ For each $j$, each $P_i$ *asserts* that $\mathsf{Verify}\big(w_j \cdot G, \mathsf{Com}_a(j), \mathsf{Com}_b(j), R_j;\ \pi_j\big)$ succeeds, identifying $P_j$ as malicious otherwise.
...
\mathsf{Com}_a = a \cdot G + \rho \cdot h_{\mathsf{ped}} \qquad
\mathsf{Com}_b = b \cdot G + \sigma \cdot h_{\mathsf{ped}}
$$

* $\mathsf{Prove}$: sample $(u_a, u_b, u_\rho, u_\sigma)$; $K_0 \gets u_a \cdot R_i + u_b \cdot G$, $K_1 \gets u_a \cdot G + u_\rho \cdot h_{\mathsf{ped}}$, $K_2 \gets u_b \cdot G + u_\sigma \cdot h_{\mathsf{ped}}$; $e \gets H(\mathsf{sid}, i, w \cdot G, \mathsf{Com}_a, \mathsf{Com}_b, R_i, K_0, K_1, K_2)$; $z_a \gets u_a + e a$, $z_b \gets u_b + e b$, $z_\rho \gets u_\rho + e \rho$, $z_\sigma \gets u_\sigma + e \sigma$; output $\pi = (e, z_a, z_b, z_\rho, z_\sigma)$.
* $\mathsf{Verify}$: $K_0 \gets z_a \cdot R_i + z_b \cdot G - e \cdot (w \cdot G)$, $K_1 \gets z_a \cdot G + z_\rho \cdot h_{\mathsf{ped}} - e \cdot \mathsf{Com}_a$, $K_2 \gets z_b \cdot G + z_\sigma \cdot h_{\mathsf{ped}} - e \cdot \mathsf{Com}_b$; accept iff $e = H(\mathsf{sid}, i, w \cdot G, \mathsf{Com}_a, \mathsf{Com}_b, R_i, K_0, K_1, K_2)$.
