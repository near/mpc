...
In this phase, a set of parties $\mathcal{P}_1 \subseteq \mathcal{P}$
of size $N_1 \geq t$ wishes to generate an EdDSA presignature. Following the
[RFC9591](https://datatracker.ietf.org/doc/html/rfc9591) we will use
domain separated hash functions $H_3$.
...
1.2 $\star$ Each $P_i$ sends $(A_i, B_i)$ **to every party**.

1.3 $\bullet$ Each $P_i$ waits to receive $(A_j, B_j)$ from every party $P_j$.

1.4 $P_i$ collects all terms into a set $\mathit{commits}\gets \set{(j, A_j, B_j)\colon \forall j \in \set{1.. N_1}}$.

**Output:** The private nonces $(a_i, b_i)$ and the public commitments $\mathit{commits}$.


## Implementation Version 2: Signing

In this phase, the same set of parties $\mathcal{P}_2 = \mathcal{P}_1$
wishes to generate an EdDSA signature. Following the
[RFC9591](https://datatracker.ietf.org/doc/html/rfc9591) we will use
...
2. The public key $X$
3. The message $m$
4. The presignature output from the Presigning phase: the private nonces $(a_i, b_i)$ and the commitments map $\mathit{commits}$.
...
1.2 Each $P_i$ sends its signature share $s_i$ **only to the coordinator**.

#### Round 1 (Coordinator)

1.3 $\bullet$ The coordinator waits to receive the signature share $s_j$ from every party $P_j$.

1.4 The coordinator runs the aggregation following [RFC9591](https://datatracker.ietf.org/doc/html/rfc9591#name-signature-share-aggregation). In short, the following sum is executed:
