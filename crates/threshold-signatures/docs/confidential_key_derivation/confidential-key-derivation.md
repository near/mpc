...
- **Publicly verifiable**: Allows any observer to verify correctness of the encrypted
  signature with respect to the MPC network public key, without knowing the
  app's secret key $a$.
...
- $`(a,A[=a \cdot G_1])`$: fresh key pair in $`\mathbb{G_1}`$ generated by *app*
  > **Publicly verifiable variant:** $`A = (A_1, A_2) = (a \cdot G_1, a \cdot G_2)`$
