use crate::{measurements::ExpectedMeasurements, report_data::ReportData};
use dcap_qvl::verify::VerifiedReport;
use interfaces::attestation::{
    AppCompose, Attestation, DstackAttestation, EventLog, MockAttestation, TcbInfo,
};
...
pub fn verify(
    ...
    match attestation {
        Attestation::Dstack(dstack_attestation) => verify_attestation(
            dstack_attestation,
        ...
        Attestation::Mock(mock_attestation) => verify_mock_attestation(
            mock_attestation,
            timestamp_seconds,
            allowed_mpc_docker_image_hashes,
            allowed_launcher_docker_compose_hashes,
...
fn verify_attestation(
    ...
    verify_tcb_status(&verification_result)
        && verify_report_data(&expected_report_data, report_data)
        && verify_static_rtmrs(report_data, &attestation.tcb_info, &expected_measurements)
        && verify_rtmr3(report_data, &attestation.tcb_info)
        && verify_app_compose(&attestation.tcb_info)
        && verify_local_sgx_digest(&attestation.tcb_info, &expected_measurements)
        && verify_mpc_hash(&attestation.tcb_info, allowed_mpc_docker_image_hashes)
    ...
    for event in filtered_events {
        ...
        match hex::decode(event.digest.as_str()) {
            Ok(decoded_digest) => {
                ...
                let expected_digest = event_digest(event.event_type, &event.event, &payload_bytes);
...
fn verify_app_compose(tcb_info: &TcbInfo) -> bool {
    ...
    let payload_is_correct = events.next().is_some_and(|event| {
        event.event_payload == tcb_info.compose_hash
            ...
            && validate_app_compose_payload(&event.event_payload, &tcb_info.app_compose)
    });
...
fn verify_mpc_hash(tcb_info: &TcbInfo, allowed_hashes: &[MpcDockerImageHash]) -> bool {
