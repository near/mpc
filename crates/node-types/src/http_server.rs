...
pub mod verifying_key_bs58 {
    ...
    fn decode_key(s: &str) -> Result<VerifyingKey, String> {
        ...
        let encoded = s
            .strip_prefix(&prefix)
            .ok_or_else(|| "missing ED25519 prefix".to_string())?;

        let bytes: [u8; 32] = bs58::decode(encoded)
            .into_vec()
            .map_err(|e| format!("base58 decode error: {e}"))?
            .try_into()
            .map_err(|v: Vec<u8>| format!("public key has unexpected length: {}", v.len()))?;
        ...
        D: Deserializer<'de>,
    {
        ...
        decode_key(&s).map_err(serde::de::Error::custom)
    }
    ...
    pub mod vec {
            ...
            D: Deserializer<'de>,
        {
            ...
            encoded
                .into_iter()
                .map(|s| decode_key(&s).map_err(serde::de::Error::custom))
                .collect()
        }
    }
}
