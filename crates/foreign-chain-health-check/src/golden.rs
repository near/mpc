//! Built-in per-network reference transactions and the value an inspector must
//! extract from each. A mainnet transaction does not exist on testnet (and vice
//! versa), so the vectors are network-specific; `None` means the chain is
//! skipped. Represented as [`HealthCheckGoldenConfig`] so callers can also
//! supply their own set from config (local chains).

use anyhow::Context;
use mpc_node_config::{AptosGolden, BlockHashGolden, HealthCheckGoldenConfig, SuiGolden};

use crate::network::Network;

pub fn golden_set(network: Network) -> HealthCheckGoldenConfig {
    match network {
...
fn block_hash_golden(tx: &str, block_hash: &str) -> Option<BlockHashGolden> {
    Some(BlockHashGolden {
        tx: tx.to_string(),
        block_hash: block_hash.to_string(),
    })
}

fn mainnet() -> HealthCheckGoldenConfig {
    HealthCheckGoldenConfig {
        base: block_hash_golden(
        ...
        abstract_chain: block_hash_golden(
            "4572b72d765f07712cf571993fd805888ede9cd05107f65338defee02f7ea755",
            "3bb255d468a552a75fc3f4916623b207ceb2d3074dfa14442ac03f0f73423708",
        ),
        bitcoin: block_hash_golden(
        ...
        aptos: Some(AptosGolden {
            tx: "adc6b85a0931fc7f0d7e3839b52d63105e22cec1cb1cdee48aa2065773098c3c".to_string(),
            event_type_tag: "0x1::block::NewBlockEvent".to_string(),
            event_sequence_number: 822_198_006,
        }),
        // The 4-byte prefix of the decoded digest is the well-known Sui chain
        // identifier — mainnet `0x35834a8a`, testnet `0x4c78adac` — which is
...
fn testnet() -> HealthCheckGoldenConfig {
    HealthCheckGoldenConfig {
        base: None,
        bnb: None,
        arbitrum: None,
        ...
        bitcoin: block_hash_golden(
            "5acaa0890f8c1f1b2ac114c25b38d376f23beda1b59e9bcba33256d6e11d7e8e",
            "000000000000021f43445ab447b3fc85e93eca26b56a4f23ef6c017682038ca2",
        ),
        starknet: block_hash_golden(
            "0x115b24c74eade5ee4c01e63cce5aa462fc2d59d040f5b088a31ad44c9aa58dc",
            "0x1f33823b145e92ca069b90d3cfb012277762d9dd1dc2efb975b10a7c3d92875",
        ),
        aptos: Some(AptosGolden {
            tx: "b463d73b3a2e9c684caf9b27eb66a147348130c50fc8fa74a3f56e712c942773".to_string(),
            event_type_tag: "0x1::block::NewBlockEvent".to_string(),
