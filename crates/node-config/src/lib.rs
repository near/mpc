...
pub use foreign_chains::{
    AuthConfig, ExpectedIdentities, ForeignChainConfig, ForeignChainHealthCheckConfig,
    ForeignChainProviderConfig, ForeignChainsConfig, TokenConfig,
};
pub use start::{
    ChainId, DownloadConfigType, GcpStartConfig, LogConfig, LogFormat, NearInitConfig,
    SecretsStartConfig, StartConfig, default_pccs_endpoints,
};
...
pub struct ConfigFile {
    ...
    /// Expected chain identities for the startup foreign-chain RPC provider health check.
    #[serde(default)]
    pub foreign_chain_health_check: ForeignChainHealthCheckConfig,
