...
impl MpcSubmitUpdateCmd {
    pub async fn run(&self, name: &str, config: ParsedConfig) {
        ...
        submitter
            ...
            .submit_update(Update::Code(contract_code))
            .await
            .into_return_value()
...
impl MpcVoteUpdateCmd {
    pub async fn run(&self, name: &str, config: ParsedConfig) {
        ...
        for account_id in from_accounts {
            ...
            futs.push(async move { handle.vote_update(update_hash).await });
        }
        ...
        for (i, result) in results.into_iter().enumerate() {
            match result.into_return_value() {
                Ok(result) => {
                    ...
                    println!(
                        "Participant {} vote_update succeeded; update approved: {}",
                        i, approved
                    );
                }
                Err(err) => {
                    println!("Participant {} vote_update failed: {:?}", i, err);
                }
            }
        }
