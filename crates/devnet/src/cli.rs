...
pub enum MpcNetworkSubCmd {
    ...
    /// Send a submit_update() transaction deploying a contract binary whose hash was voted in.
    SubmitUpdate(MpcSubmitUpdateCmd),
    /// Send vote_update() transactions to vote for the hash of a new contract binary.
...
pub struct MpcVoteNewParametersCmd {
    ...
    /// participant count.
    #[clap(long = "per-domain-threshold", value_delimiter = ',', value_parser = parse_per_domain_threshold)]
