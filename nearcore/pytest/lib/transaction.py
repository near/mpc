...
def make_transaction(
    receiverId, nonce, actions, blockHash, accountId, pk
) -> Transaction:
    tx = Transaction()
...
def sign_transaction(
    receiverId, nonce, actions, blockHash, accountId, pk, sk
) -> SignedTransaction:
...
def sign_and_serialize_transaction(
    ...
    return serialize_transaction(
        sign_transaction(receiverId, nonce, actions, blockHash, accountId, pk, sk)
    )


def compute_delegated_action_hash(
...
def create_signed_delegated_action(
    senderId, receiverId, actions, nonce, maxBlockHeight, publicKey, sk
):
...
def sign_delegate_action(signedDelegate, signer_key, contract_id, nonce, blockHash):
...
def sign_create_account_tx(creator_key, new_account_id, nonce, block_hash) -> bytes:
...
def sign_delete_access_key_tx(
    signer_key, target_account_id, key_for_deletion, nonce, block_hash
):
...
def sign_payment_tx_and_get_hash(key, to, amount, nonce, block_hash):
    action = create_payment_action(amount)
    tx = make_transaction(
        to, nonce, [action], block_hash, key.account_id, key.decoded_pk()
    )
    hash_bytes = compute_transaction_hash(tx)
...
def sign_staking_tx_and_get_hash(signer_key, validator_key, amount, nonce, block_hash):
    action = create_staking_action(amount, validator_key.decoded_pk())
    tx = make_transaction(
        signer_key.account_id,
        nonce,
    ...
    hash_bytes = compute_transaction_hash(tx)
    signed_tx = sign_staking_tx(signer_key, validator_key, amount, nonce, block_hash)
    return signed_tx, base58.b58encode(hash_bytes).decode("utf8")
...
def sign_deploy_contract_transaction(
    signer_key, code, nonce, blockHash
) -> SignedTransaction:
...
def sign_deploy_global_contract_transaction(
    signer_key, code, deployMode, nonce, blockHash
) -> SignedTransaction:
...
def sign_use_global_contract_transaction(
    signer_key, identifier, nonce, blockHash
) -> SignedTransaction:
...
def sign_deploy_global_contract_tx(
    signer_key, code, deployMode, nonce, blockHash
) -> bytes:
    tx = sign_deploy_global_contract_transaction(
        signer_key, code, deployMode, nonce, blockHash
    )
    return serialize_transaction(tx)
...
def sign_function_call_transaction(
    signer_key, contract_id, methodName, args, gas, deposit, nonce, blockHash
) -> SignedTransaction:
...
def sign_function_call_tx(
    ...
    tx = sign_function_call_transaction(
        signer_key, contract_id, methodName, args, gas, deposit, nonce, blockHash
    )
    return serialize_transaction(tx)
