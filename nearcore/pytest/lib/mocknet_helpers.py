...
def get_nonce_for_pk(
    account_id, pk, finality="optimistic", addr=LOCAL_ADDR, port=RPC_PORT, logger=logger
):
...
def throttle_txns(send_txns, total_tx_sent, elapsed_time, test_state):
    ...
    excess_transactions = total_tx_sent - (
        test_state.max_tps_per_node * (elapsed_time + duration)
    )
...
def tx_result(txn_id, account_id, wait_for_success=False, **kwargs):
    while True:
        status = json_rpc("EXPERIMENTAL_tx_status", [txn_id, account_id], **kwargs)
        if "error" in status:
