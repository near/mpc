...
def setup_python_environment(node, wasm_contract):
    ...
    m.upload(
        wasm_contract, os.path.join(PYTHON_DIR, WASM_FILENAME), switch_user="ubuntu"
    )
...
def start_load_test_helper_script(
    ...
    logger.info(f"Starting load test helper. Node account id: {node_account_id}.")
...
def get_chunk_txn(index, chunks, archival_node, result):
    ...
    result[index] = len(archival_node.get_chunk(chunk_hash)["result"]["transactions"])
...
def chain_measure_bps_and_tps(
    ...
    logger.info(f"Measuring BPS and TPS in the time range {start_time} to {end_time}")
    ...
    while curr_time > start_time:
        if curr_time < end_time:
            ...
            pmap(
                lambda i: get_chunk_txn(
                    i, curr_block["chunks"], archival_node, tx_per_chunk
                ),
...
def send_transaction(node, tx, tx_hash, account_id, timeout=120):
    ...
    while "error" in response.keys():
        ...
        if "timeout" in error_data.lower():
            ...
            raise RuntimeError(f"Error in processing transaction {tx_hash}: {response}")
    ...
    if "SuccessValue" not in response["result"]["status"]:
        raise RuntimeError(f"ERROR: Failed transaction {tx_hash}. Response: {response}")
...
def transfer_between_nodes(nodes):
    ...
    tx, tx_hash = sign_payment_tx_and_get_hash(
        alice, bob.account_id, transfer_amount, alice_nonce + 1, last_block_hash
    )
    ...
    assert (alice_initial_balance - alice_final_balance) % 1000 == transfer_amount
...
def get_near_pid(machine):
    p = machine.run("ps aux | grep 'near.* run' | grep -v grep | awk '{print $2}'")
...
def upload_and_extract(node, src_filename, dst_filename):
    node.machine.upload(
        f"{src_filename}.gz", f"{dst_filename}.gz", switch_user="ubuntu"
    )
    node.machine.run(
        "gunzip -f {dst_filename}.gz".format(dst_filename=shlex.quote(dst_filename))
    )
...
def compress_and_upload(nodes, src_filename, dst_filename):
    ...
    pmap(lambda node: upload_and_extract(node, src_filename, dst_filename), nodes)


# cspell:ignore redownload
def redownload_neard(nodes, binary_url):
    pmap(
        lambda node: node.machine.run(
            "sudo -u ubuntu -i",
            input='wget -O /home/ubuntu/neard "{}"; chmod +x /home/ubuntu/neard'.format(
                binary_url
            ),
        ),
        nodes,
...
def create_and_upload_genesis(
    ...
    assert "-spoon" in chain_id, (
        f'Expecting chain_id like "testnet-spoon" or "mainnet-spoon", got {chain_id}'
    )
    ...
    pmap(
        lambda node: wait_genesis_updater_done(node, done_filename),
        validator_nodes + rpc_nodes,
    )


def extra_genesis_records(
    validator_keys,
    rpc_node_names,
    node_pks,
    seen_accounts,
...
def neard_amend_genesis(
    ...
    extra_records, validators = extra_genesis_records(
        validator_keys, rpc_node_names, node_pks, set(), num_seats, increasing_stakes
    )
...
def create_and_upload_genesis_file_from_empty_genesis(
    ...
    node0.machine.run(
        "rm -rf /home/ubuntu/.near-tmp && mkdir /home/ubuntu/.near-tmp && /home/ubuntu/neard --home /home/ubuntu/.near-tmp init --chain-id {}".format(
            chain_id
        )
    )
    ...
    pmap(
        lambda node: upload_json(
            node, "/home/ubuntu/.near/genesis.json", genesis_config
        ),
...
def get_node_addr(node, port):
    node_key_json = download_and_read_json(node, "/home/ubuntu/.near/node_key.json")
...
def neard_start_script(node, upgrade_schedule=None, epoch_height=None):
    if upgrade_schedule and upgrade_schedule.get(node.instance_name, 0) <= epoch_height:
...
def reset_data(node, retries=0):
    try:
        ...
        if retries < 3:
            logger.warning("And error occurred while clearing data directory, retrying")
