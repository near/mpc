...
def proxy_cleanup(proxy):
    ...
    if proxy.error.value != 0:
        assert False, (
            "One of the proxy processes failed, search for the stacktraces above"
        )
        ...
        try:
            ...
            if raw_message[0] == 13:
                ...
                if raw_message[ser.offset] in [3, 4, 5, 7]:
                    ...
                    return (
                        await self.handle(None, sender_ordinal, receiver_ordinal)
                        is not False
                    )
                logger.info(f"ERROR 13 {int(raw_message[ser.offset])} exception={e}")
...
class NodesProxy:
    ...
    def proxify_node(self, node):
        proxify_node(node, self.ps, self.handler, self.global_stopped, self.error, self)
...
async def bridge(
    reader, writer, handler_fn, global_stopped, local_stopped, bridge_stopped, error
):
    ...
    try:
        while (
            0 == global_stopped.value
            and 0 >= local_stopped.value
            and 0 == error.value
            and 0 == bridge_stopped[0]
        ):
        ...
        logging.debug(f"Gracefully close bridge. port={_MY_PORT} bridge_id={bridge_id}")
...
async def listener(
    inner_port, outer_port, handler_ctr, global_stopped, local_stopped, error
):
