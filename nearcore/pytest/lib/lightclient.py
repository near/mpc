...
def validate_light_client_block(
    last_known_block, new_block, block_producers_map, panic=False
):
    new_block_hash = compute_block_hash(
        new_block["inner_lite"],
    ...
    for approval, stake in zip(new_block["approvals_after_next"], block_producers):
    ...
    if (
        ...
        for bp in new_block["next_bps"]:
            ...
            serialized_next_bp += base58.b58decode(bp["public_key"][len(ED_PREFIX) :])
            stake = int(bp["stake"])
            for i in range(16):
        ...
        computed_hash = base58.b58encode(hashlib.sha256(serialized_next_bp).digest())
        if computed_hash != new_block["inner_lite"]["next_bp_hash"].encode("utf-8"):
            if panic:
                assert False
            return False

        block_producers_map[new_block["inner_lite"]["next_epoch_id"]] = new_block[
            "next_bps"
        ]
