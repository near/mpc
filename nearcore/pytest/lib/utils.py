...
class MetricsTracker:
    ...
    def get_all_metrics(self) -> str:
        ...
        if not response.ok:
            raise RuntimeError(f"Could not fetch metrics from {self.addr}: {response}")
    ...
    def get_metric_all_values(
        ...
        for family in text_string_to_metric_families(self.get_all_metrics()):
            if family.name == metric_name:
                return [(sample.labels, sample.value) for sample in family.samples]
        return []

    def get_metric_value(
        self, metric_name: str, labels: typing.Optional[typing.Dict[str, str]] = None
    ) -> typing.Optional[str]:
...
def collect_gcloud_config(num_nodes):
    ...
    for i in range(num_nodes):
        ...
        if not node_dir.exists():
            ...
            for filename in (
                ...
                host.download(
                    f"/home/bowen_nearprotocol_com/.near/{filename}", str(node_dir)
                )
    ...
    with open(tempdir / "node0" / "config.json") as f:
        config = json.load(f)
    ip_addresses = map(
        lambda x: x.split("@")[-1], config["network"]["boot_nodes"].split(",")
    )
    res = {
        "nodes": list(
            map(lambda x: {"ip": x.split(":")[0], "port": 3030}, ip_addresses)
        ),
        "accounts": keys,
    }
...
def obj_to_string(obj, extra="    ", full=False):
    if type(obj) in [tuple, list]:
        return (
            "tuple"
            + "\n"
            + "\n".join((extra + obj_to_string(x, extra + "    ")) for x in obj)
        )
...
def poll_epochs(
    ...
    *,
    ...
    if count:
        msg += (
            f"First epoch: {start_height}; last epoch: {previous}\n"
...
def poll_blocks(
    ...
    **kw,
) -> typing.Iterable[cluster.BlockId]:
    ...
    if count > 0:
        msg += (
            f"First block: {start_height}; last block: {previous}\n"
...
def figure_out_sandbox_binary():
    ...
    for release in ("release", "debug"):
        ...
        for exe in ("neard-sandbox", "near-sandbox", "neard"):
            if (root / exe).exists():
                logger.info(f"Using {(root / exe).relative_to(repo_dir)} binary")
                config["near_root"] = str(root)
                config["binary_name"] = exe
                return config

    assert False, (
        "Unable to figure out location of neard-sandbox binary; "
