...
class BlockId(typing.NamedTuple):
    ...
    def __eq__(self, rhs) -> bool:
        return (
            isinstance(rhs, BlockId)
            and self.height == rhs.height
            and self.hash == rhs.hash
        )
...
class BaseNode(object):
    ...
    def _get_command_line(
        self, near_root, node_dir, boot_node: BootNode, binary_name="neard"
    ):
    ...
    def get_command_for_subprogram(self, cmd: tuple):
        return (
            os.path.join(self.near_root, self.binary_name),
            "--home",
            self.node_dir,
        ) + cmd
    ...
    def get_status(
        self, check_storage: bool = True, timeout: float = 4, verbose: bool = False
    ):
    ...
    def get_latest_block(self, **kw) -> BlockId:
        ...
        return BlockId(
            height=sync_info["latest_block_height"], hash=sync_info["latest_block_hash"]
        )
    ...
    def get_account(
        self, acc, finality="optimistic", block=None, do_assert=True, **kwargs
    ):
        query = {
    ...
    def get_epoch_id(
        ...
        """
        assert block_height is None or block_hash is None, (
            "use either height or has, not both"
        )
    ...
    def stop_checking_store(self):
        logger.warning(
            "Stopping checking Storage for inconsistency for %s:%s" % self.addr()
        )
...
class LocalNode(BaseNode):
    def __init__(
        ...
        self.change_config(
            {
                "network": {"addr": f"0.0.0.0:{port}", "blacklist": list(blacklist)},
                "rpc": {
                    "addr": f"0.0.0.0:{rpc_port}",
                },
                "consensus": {"min_num_peers": int(not single_node)},
            }
        )
    ...
    def start(
        ...
        *,
        ...
        try:
            ...
            logger.error("=== failed to start node, rpc is not ready in 10 seconds")

    def run_cmd(self, *, cmd: tuple, extra_env: typing.Dict[str, str] = dict()):
        env = os.environ.copy()
        env["RUST_BACKTRACE"] = "1"
        env["RUST_LOG"] = (
            "actix_web=warn,mio=warn,tokio_util=warn,actix_server=warn,actix_http=warn,"
        ...
        with (
            open(self.stdout_name, "ab") as stdout,
...
class GCloudNode(BaseNode):
    def __init__(self, *args, username=None, project=None, ssh_key_path=None):
        if len(args) == 1:
            ...
            self.port = 24567
            self.rpc_port = 3030
            self.machine = gcloud.get(
                name, username=username, project=project, ssh_key_path=ssh_key_path
            )
    ...
    def _upload_config_files(self, node_dir):
        ...
        self.machine.upload(
            os.path.join(node_dir, "*.json"), f"/home/{self.machine.username}/.near/"
        )
        self.validator_key = Key.from_json_file(
            os.path.join(node_dir, "validator_key.json")
        )
...
""",
    ...
    def start(
        self, *, boot_node: BootNode = None, extra_env: typing.Dict[str, str] = dict()
    ):
        ...
        self.machine.run_detach_tmux(
            extra_env + " ".join(self._get_command_line(".", ".near", boot_node))
        )
    ...
    def cleanup(self):
        ...
        self.machine.download(
            "/tmp/python-rc.log", f"/tmp/pytest_remote_log/{self.machine.name}.log"
        )
    ...
    def reset_validator_key(self, new_key):
        ...
        with open(os.path.join(self.node_dir, "validator_key.json"), "w+") as f:
            json.dump(new_key.to_json(), f)
        self.machine.upload(
            os.path.join(self.node_dir, "validator_key.json"),
...
def spin_up_node(
    ...
    *,
    ...
    logger.info(
        "Starting node %s %s"
        % (ordinal, ("with " + "=".join(args) if args else "as BOOT NODE"))
    )
    if is_local:
        blacklist = [
            "127.0.0.1:%s" % (24567 + 10 + bl_ordinal) for bl_ordinal in blacklist
        ]
        node = LocalNode(
            24567 + 10 + ordinal,
            3030 + 10 + ordinal,
        ...
        instance_name = "{}-{}-{}".format(
            config["remote"].get("instance_name", "near-pytest"), ordinal, uuid.uuid4()
        )
...
def init_cluster(
    ...
    logger.info(
        "Creating %s cluster configuration with %s nodes"
    ...
    assert len(node_dirs) == num_nodes + num_observers, (
        "node dirs: %s num_nodes: %s num_observers: %s"
    ...
    if extra_state_dumper:
        (node_config_dump, node_config_sync) = (
            state_sync_lib.get_state_sync_configs_pair(tracked_shards_config=None)
        )
...
def configure_cold_storage_for_archival_node(node_dir: str):
    ...
    if (
        not config_json.get("archive", False)
        or config_json.get("cold_store") is not None
    ):
        return
    ...
    if "split_storage" not in config_json:
        config_json["split_storage"] = {
            ...
            "cold_store_loop_sleep_duration": {"secs": 0, "nanos": 100000000},
        }
...
def apply_genesis_changes(node_dir: str, genesis_config_changes: GenesisConfigChanges):
...
def apply_config_changes(node_dir: str, client_config_change: ClientConfigChange):
...
def start_cluster(
    ...
    for i in range(1, len(node_dirs)):
        handle = threading.Thread(target=spin_up_node_and_push, args=(i, boot_node))
        handle.start()
