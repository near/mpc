...
class SimpleTransferBetweenAccounts:
    ...
    def retrieve_nonces(self, signer_keys):
        ...
        for signer_key in signer_keys:
            ...
            nonce = (
                self.rpc_node.get_nonce_for_pk(signer_key.account_id, signer_key.pk) + 1
            )
        ...
        while True:
            ...
            if random.random() < 0.5:
                ...
                payment_tx = transaction.sign_payment_tx(
                    ...
                    to_account_id,
                    1,
                    self.next_nonce(from_account_key),
                    last_block_hash,
                )
    ...
    def deploy_contracts(self):
        ...
        assert len(self.account_keys) > 0, (
            "Accounts not initialized, call create_accounts first."
        )
        ...
        for account_key in self.account_keys:
            ...
            deploy_contract_tx = transaction.sign_deploy_contract_tx(
                account_key, contract, self.next_nonce(account_key), last_block_hash
            )
        ...
        self.wait_for_txs(assert_all_accepted=True, tx_list=deploy_contract_tx_list)
    ...
    def create_accounts(self):
        ...
        for i in range(len(self.account_prefixes)):
            ...
            tx_hash = self.create_account(account_key, 1000 * ONE_NEAR, signer_key)
            create_account_tx_list.append((signer_key.account_id, tx_hash))
            logger.info(f"Creating account: {account_key.account_id}, tx: {tx_hash}")

        self.wait_for_txs(assert_all_accepted=True, tx_list=create_account_tx_list)
    ...
    def wait_for_txs(
        self, assert_all_accepted: bool, tx_list: list[(AccountId, TxHash)] = None
    ):
    ...
    def wait_for_blocks(self, num_blocks, timeout=None):
        height, _ = utils.wait_for_blocks(
            self.rpc_node, count=num_blocks, timeout=timeout
        )
        return height
