...
class Executables(typing.NamedTuple):
    ...
    def node_config(self) -> typing.Dict[str, typing.Any]:
        return {"local": True, "near_root": self.root, "binary_name": self.neard.name}
...
def _compile_current(branch: str) -> Executables:
    ...
    if prebuilt_neard is not None:
        logger.info(f"Using `CURRENT_NEARD={prebuilt_neard}` neard for branch {branch}")
        try:
    ...
    subprocess.check_call(
        ["cargo", "build", "-p", "neard", "--bin", "neard"], cwd=_REPO_DIR
    )
    subprocess.check_call(
        ["cargo", "build", "-p", "near-test-contracts"], cwd=_REPO_DIR
    )
...
def patch_binary(binary: pathlib.Path) -> None:
    ...
    """
    path = subprocess.run(
        ("nix-build", "-E", nix_expr), capture_output=True, encoding="utf-8"
    ).stdout.strip()
    # Set the interpreter for the binary to NixOS'.
    patchelf = f"{path}/bin/patchelf"
    linker = (pathlib.Path(path) / "nix-support" / "dynamic-linker").read_text().strip()
...
def prepare_ab_test(chain_id: str = "mainnet") -> ABExecutables:
    ...
    return ABExecutables(stable=stable, current=current, release=release, deploy=deploy)
...
def __get_executables_for(chain_id: str) -> typing.Tuple[str, str, Executables]:
    ...
    if chain_id not in ("mainnet", "testnet", "betanet"):
        raise ValueError(
            f"Unexpected chain_id: {chain_id}; expected mainnet, testnet or betanet"
        )
