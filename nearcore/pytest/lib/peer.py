...
class Connection:
    ...
    async def recv(self, expected=None):
        while True:
            ...
            if (
                expected is None
                or response.enum == expected
                or (callable(expected) and expected(response))
            ):
                return response
...
async def run_handshake(
    ...
    if (
        response.enum == "HandshakeFailure"
        and response.HandshakeFailure[1].enum == "ProtocolVersionMismatch"
    ):
    ...
    if (
        response.enum == "HandshakeFailure"
        and response.HandshakeFailure[1].enum == "GenesisMismatch"
    ):
    ...
    assert response.enum == "Handshake", (
        response.enum
        if response.enum != "HandshakeFailure"
        else response.HandshakeFailure[1].enum
    )
