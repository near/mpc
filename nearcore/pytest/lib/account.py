...
class Account:
    def __init__(self, key, init_nonce, base_block_hash, rpc_info=None, rpc_infos=None):
        """
    ...
    def send_tx(self, signed_tx):
        return self.json_rpc(
            "broadcast_tx_async", [base64.b64encode(signed_tx).decode("utf-8")]
        )

    def send_tx_sync(self, signed_tx):
        return self.json_rpc(
            "broadcast_tx_commit", [base64.b64encode(signed_tx).decode("utf-8")]
        )
    ...
    def send_transfer_tx(
        self, dest_account_id, transfer_amount=100, base_block_hash=None
    ):
        self.prep_tx()
        tx = sign_payment_tx(
            self.key,
    ...
    def send_deploy_contract_tx(self, wasm_filename, base_block_hash=None):
        ...
        tx = sign_deploy_contract_tx(
            self.key, wasm_binary, self.nonce, base_block_hash or self.base_block_hash
        )
        return self.send_tx(tx)

    def send_call_contract_tx(self, method_name, args, base_block_hash=None):
        return self.send_call_contract_raw_tx(
            self.key.account_id, method_name, args, 0, base_block_hash=base_block_hash
        )

    def send_call_contract_raw_tx(
        self, contract_id, method_name, args, deposit, base_block_hash=None
    ):
        self.prep_tx()
    ...
    def send_call_contract_raw_tx_sync(
        self, contract_id, method_name, args, deposit, base_block_hash=None
    ):
        self.prep_tx()
        tx = sign_function_call_tx(
