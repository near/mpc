...
def create_subaccounts(node, signer_key):
    ...
    for c in range(ord("a"), ord("z")):
        ...
        sub_key = mirror_utils.create_subaccount(
            node, subaccount, signer_key.key, nonce, block_hash_bytes
        )
    ...
    for height, block_hash in utils.poll_blocks(node, timeout=200):
        ...
        for i, account in enumerate(implicit_accounts):
            if not add_key_sent[i]:
                ...
                if nonce is None:
                    continue
                mirror_utils.send_add_access_key(
                    node, account.signer_key(), keys[i], nonce, block_hash_bytes
                )
                add_key_sent[i] = True
...
def copy_near_home(source_home_dir, target_home_dir):
    ...
    with (
        open(source_home_dir / "config.json") as source_f,
    ...
    shutil.copyfile(source_home_dir / "genesis.json", target_home_dir / "genesis.json")
...
def run_forked_network(fork_dir, config, validator_key, num_original_nodes):
    ...
    validator = spin_up_node(config, config["near_root"], str(validator_home), node_idx)

    node_idx += 1
    rpc = spin_up_node(
        config, config["near_root"], str(rpc_home), node_idx, boot_node=validator
    )
...
def main():
    ...
    nodes = start_cluster(
        ...
        client_config_changes={
            1: {"tracked_shards_config": "AllShards"},
            2: {"tracked_shards_config": "AllShards"},
        },
    )
    ...
    neard_path = pathlib.Path(config["near_root"]) / pathlib.Path(config["binary_name"])
