...
CONTRACT_PATH = (
    ...
    / "contract/target/wasm32-unknown-unknown/release/addkey_contract.wasm"
)
...
def init_target_dir(neard, home, ordinal, boot_node_home, validator_account=None):
    mkdir_clean(home)
...
def init_target_dirs(neard, last_ordinal, target_validators):
    ...
    for i in range(len(target_validators)):
        ...
        dirs.append(home)
        init_target_dir(
            neard, home, ordinal, boot_node_home, validator_account=account_id
        )
        ordinal += 1
...
def create_forked_chain(config, near_root, source_node_homes, target_validators):
...
class MirrorProcess:
    ...
    def start(self):
        env = os.environ.copy()
        env["RUST_LOG"] = (
            "actix_web=warn,mio=warn,tokio_util=warn,actix_server=warn,actix_http=warn,indexer=info,"
            + env.get("RUST_LOG", "debug")
        )
        config_path = dot_near() / f"{MIRROR_DIR}/config.json"
        with (
            open(dot_near() / f"{MIRROR_DIR}/stdout", "ab") as stdout,
            open(dot_near() / f"{MIRROR_DIR}/stderr", "ab") as stderr,
            open(config_path, "w") as mirror_config,
        ):
            json.dump(
                {"tx_batch_interval": {"secs": 0, "nanos": 600000000}}, mirror_config
            )
            args = [
                self.neard,
                "mirror",
            ...
            self.process = subprocess.Popen(
                args, stdin=subprocess.DEVNULL, stdout=stdout, stderr=stderr, env=env
            )
...
def send_add_access_key(node, key, target_key, nonce, block_hash):
    ...
    res = node.send_tx(tx)
    logger.info(f"sent add key tx for {target_key.account_id} {target_key.pk}: {res}")


def send_delete_access_key(node, key, target_key, nonce, block_hash):
    action = transaction.create_delete_access_key_action(target_key.decoded_pk())
    tx = transaction.sign_and_serialize_transaction(
        target_key.account_id,
        nonce,
        [action],
...
def create_subaccount(
    node, subaccount_name, signer_key, nonce, block_hash, extra_key=False
):
...
def deploy_addkey_contract(node, signer_key, contract_path, nonce, block_hash):
    ...
    tx = transaction.sign_deploy_contract_tx(signer_key, code, nonce, block_hash)
    res = node.send_tx(tx)
...
def call_create_account(node, signer_key, account_id, public_key, nonce, block_hash):
    args = json.dumps({"account_id": account_id, "public_key": public_key})
    args = bytearray(args, encoding="utf-8")

    actions = [
        transaction.create_function_call_action("create_account", args, 10**14, 10**24),
        transaction.create_payment_action(123),
    ]
    tx = transaction.sign_and_serialize_transaction(
        "test0",
...
def contract_deployed(node, account_id):
    return "error" not in node.json_rpc(
        "query",
        {"request_type": "view_code", "account_id": account_id, "finality": "final"},
    )
...
class AddedKey:
    ...
    def get_nonce(self, node):
        if self.nonce is not None:
            self.nonce += 1
            return self.nonce
        self.nonce = node.get_nonce_for_pk(
            self.key.account_id, self.key.pk, finality="final"
        )
        if self.nonce is not None:
            logger.info(
    ...
    def send_if_inited(self, node, transfers, block_hash):
        for receiver_id, amount in transfers:
            ...
            tx = transaction.sign_payment_tx(
                self.key, receiver_id, amount, self.nonce, block_hash
            )
            node.send_tx(tx)
...
class ImplicitAccount:
    ...
    def transfer_to(self, node, sender_key, amount, block_hash, nonce):
        tx = transaction.sign_payment_tx(
            sender_key, self.account_id(), amount, nonce, block_hash
        )
        node.send_tx(tx)
        logger.info(f"sent {amount} to initialize implicit account {self.account_id()}")
...
def check_num_txs(source_node, target_node):
    ...
    assert total_source_txs <= total_target_txs, (total_source_txs, total_target_txs)
    logger.info(
...
def added_keys_send_transfers(nodes, added_keys, receivers, amount, block_hash):
    ...
    for key in added_keys:
        key.send_if_inited(
            nodes[node_idx], [(receiver, amount) for receiver in receivers], block_hash
        )
        node_idx += 1
        node_idx %= len(nodes)
...
def start_source_chain(config, num_source_validators=3):
    ...
    for i in range(num_source_validators + 1):
        config_changes[i] = {"tracked_shards_config": "AllShards", "archive": True}
    ...
    for i in range(1, num_source_validators):
        source_nodes.append(
            spin_up_node(
                config, near_root, source_node_dirs[i], i, boot_node=source_nodes[0]
            )
        )
    traffic_data = TrafficData(len(source_nodes))

    traffic_data.implicit_account = ImplicitAccount()
    for height, block_hash in utils.poll_blocks(source_nodes[0], timeout=TIMEOUT):
        block_hash_bytes = base58.b58decode(block_hash.encode("utf8"))
        traffic_data.implicit_account.transfer_to(
            source_nodes[0],
        ...
        deploy_addkey_contract(
            source_nodes[0],
            source_nodes[1].signer_key,
            CONTRACT_PATH,
            traffic_data.nonces[1],
            block_hash_bytes,
        )
    ...
    for height, block_hash in utils.poll_blocks(source_nodes[0], timeout=TIMEOUT):
        ...
        traffic_data.implicit_account.send_if_inited(
            source_nodes[0], [("test2", height), ("test3", height)], block_hash_bytes
        )
    ...
    source_nodes[0].kill()
    target_node_dirs = create_forked_chain(
        config, near_root, source_node_dirs, TARGET_VALIDATORS
    )
...
def send_traffic(near_root, source_nodes, traffic_data, callback):
    ...
    for height, block_hash in utils.poll_blocks(source_nodes[1], timeout=TIMEOUT):
        if not callback():
            break
        block_hash_bytes = base58.b58decode(block_hash.encode("utf8"))
        ...
        if test0_deleted_height is None:
            ...
            traffic_data.send_transfers(
                source_nodes, block_hash_bytes, skip_senders=set([0])
            )
        ...
        added_keys_send_transfers(
            source_nodes,
            keys,
            [
                traffic_data.implicit_account.account_id(),
                implicit_account2.account_id(),
                "test2",
                "test3",
            ],
            height,
        ...
        if implicit_added is None:
            ...
            if implicit_account2.inited() and height - start_source_height >= 15:
            ...
            implicit_added.send_if_inited(
                source_nodes[1], [("test0", height)], block_hash_bytes
            )
            if implicit_added.inited() and not implicit_deleted:
                send_delete_access_key(
                    source_nodes[1],
                    implicit_added.key,
                    implicit_account2.key.key,
                    implicit_added.nonce,
                    block_hash_bytes,
                )
        ...
        if (
            test0_re_added_key is None
            and test0_deleted_height is not None
            and height - test0_deleted_height >= 5
        ):
            send_add_access_key(
                source_nodes[1],
                new_key.key,
        ...
        if test0_re_added_key is not None:
            test0_re_added_key.send_if_inited(
                source_nodes[1],
                [("test3", height), (implicit_account2.account_id(), height)],
                block_hash_bytes,
            )
        ...
        if subaccount_key.inited():
            if not subaccount_contract_deployed:
                ...
                if contract_deployed(source_nodes[0], subaccount_key.account_id()):
                    subaccount_key.nonce += 1
                    call_stake(
                        source_nodes[0],
                        subaccount_key.key,
