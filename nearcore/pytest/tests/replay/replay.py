...
def convert_json_public_key_to_py_public_key(json_public_key):
    pk_str = (
        json_public_key.split(":")[1] if ":" in json_public_key else json_public_key
    )
    ans = PublicKey()
    ans.keyType = 0
...
def send_resigned_transactions(tx_path, home_dir):
    ...
    key_pair = Key(
        key_pair_json["account_id"],
        key_pair_json["public_key"],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tx-json", type=str, required=False, help="Path of tx history json"
    )
    parser.add_argument("--genesis", type=str, required=False, help="Path of genesis")
    parser.add_argument(
        "--home-dir", type=str, required=True, help="Path of the new home directory"
    )
