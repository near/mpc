...
def main():
    ...
    epoch = state_sync_lib.approximate_epoch_height(latest_block.height, EPOCH_LENGTH)
    ...
    tx = transaction.sign_payment_tx(
        boot_node.signer_key, test_account_id, 1, nonce, latest_block_hash
    )
    logger.info(
    ...
    logger.info(f"The non-validator node is at block height {node_latest_block_height}")

    # Check that the non-validator node is not stuck, and is in-sync.
    assert (
        boot_node_latest_block_height
        < int(0.5 * EPOCH_LENGTH) + node_latest_block_height
    )
