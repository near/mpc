...
def assert_roles_as_expected(rpc_node, hash):
    ...
    (_kickouts, current_validators, _next_validators) = get_epoch_info(rpc_node, hash)
...
_height, hash = utils.wait_for_blocks(rpc_node, target=EPOCH_LENGTH + EPOCH_LENGTH // 3)
...
assert find_account(next_validators, chunk_validator_account) is not None, (
    f"{chunk_validator_account} must be in next validators"
)
