...
validator = spin_up_node(config, near_root, node_dirs[1], 1, boot_node=boot_node)
delayed_validator = spin_up_node(
    config, near_root, node_dirs[2], 2, boot_node=boot_node
)
...
def wait_for_height(target_height, rpc_node, sleep_time=2, bps_threshold=-1):
    ...
    while latest_height < target_height:
        ...
        try:
            new_height = rpc_node.get_latest_block(
                check_storage=False, timeout=10
            ).height
