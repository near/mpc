...
from transaction import (
    ...
    sign_payment_tx,
    sign_deploy_contract_to_new_account_tx,
)
import utils
...
def get_proto_version(exe: pathlib.Path) -> (int, int):
    ...
    assert m, (
        f"Unable to extract protocol version number from {exe};\n"
...
class TestUpgrade:
    ...
    def run(self) -> None:
        ...
        traffic_generator.deploy_test_contracts(self._executables.current.node_config())
    ...
    def upgrade_nodes(self) -> None:
        ...
        for node in self._stable_nodes:
            ...
            node.start(
                ...
                extra_env={"NEAR_TESTS_PROTOCOL_UPGRADE_OVERRIDE": "sequential"},
            )
    ...
    def wait_for_no_missed_endorsements(self) -> None:
        def _condition():
            ...
            for i in range(self._num_validators):
                ...
                assert num_missed == 0, (
                    f"validator {validator_id} missed {num_missed} endorsements"
                )
