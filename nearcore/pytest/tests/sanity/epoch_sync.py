...
class EpochSyncTest(unittest.TestCase):
    def setUp(self):
        ...
        node_config["epoch_sync"] = {
            ...
            "timeout_for_epoch_sync": {"secs": 5, "nanos": 0},
        }
        ...
        self.near_root, self.node_dirs = init_cluster(
            ...
            genesis_config_changes=[
                ...
                ["transaction_validity_period", 2 * EPOCH_LENGTH],
            ],
    ...
    def test(self):
        ...
        node1 = spin_up_node(
            self.config, self.near_root, self.node_dirs[1], 1, boot_node=node0
        )

        ctx = utils.TxContext([0, 0], [node0, node1])

        for height, block_hash in utils.poll_blocks(
            node0, timeout=SYNC_FROM_BLOCK * 2, poll_interval=0.1
        ):
        ...
        node2 = spin_up_node(
            self.config, self.near_root, self.node_dirs[2], 2, boot_node=node0
        )
        tracker = utils.LogTracker(node2)

        utils.wait_for_blocks(
            node2, target=CATCHUP_BLOCK, timeout=(CATCHUP_BLOCK - SYNC_FROM_BLOCK) * 2
        )
