...
config_overrides = {
    ...
    "consensus": {
        "state_sync_external_timeout": {"secs": 0, "nanos": 500000000},
        "state_sync_p2p_timeout": {"secs": 0, "nanos": 500000000},
        "state_sync_external_backoff": {"secs": 0, "nanos": 500000000},
    },
}
...
for largest_height in range(5, HEIGHT_GOAL + 1):
    ...
    block = wait_until_available(
        lambda: nodes[0].get_block_by_height(largest_height, timeout=5), BLOCK_QUERY
    )
    ...
    if epoch_id not in seen_epochs:
        ...
        if height - 1 in blocks_by_height:
            ...
            assert (
                prev_block["result"]["header"]["epoch_id"]
                != block["result"]["header"]["epoch_id"]
            )
