...
observed_height = 0
for observed_height, hash_ in utils.poll_blocks(
    boot_node, timeout=TIMEOUT, poll_interval=0.1
):
...
catch_up_height = 0
for catch_up_height, hash_ in utils.poll_blocks(
    node2, timeout=TIMEOUT, poll_interval=0.1
):
...
tracker.offset = (
    0  # the transition might have happened before we initialized the tracker
)
