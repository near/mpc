    ...
    def test(self):
        ...
        node_client_config_changes = {
            i: {
                ...
                **consensus_config_changes(
                    min_block_production_delay_seconds=0.5,
        ...
        for i, node in enumerate(nodes):
            node.change_config(
                consensus_config_changes(
                    min_block_production_delay_seconds=2.0,
        ...
        wait_for_consecutive_blocks(
            rpc_node, target=2, chunk_mask_condition=lambda mask: all(mask)
        )


def consensus_config_changes(
    min_block_production_delay_seconds: float,
    max_block_production_delay_seconds: float,
    max_block_wait_delay_seconds: float,
):
    return {
        "consensus.min_block_production_delay": to_config_duration(
            min_block_production_delay_seconds
        ),
        "consensus.max_block_production_delay": to_config_duration(
            max_block_production_delay_seconds
        ),
        "consensus.max_block_wait_delay": to_config_duration(
            max_block_wait_delay_seconds
        ),
    }
