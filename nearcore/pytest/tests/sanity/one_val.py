...
genesis_change = [
    ...
    ["records", 0, "Account", "account", "locked", "110000000000000000000000000000000"],
...
for height, hash_ in utils.poll_blocks(nodes[0], timeout=TIMEOUT, poll_interval=0.1):
    logger.info(f"Got to height {height}")

    if ctx.get_balances() == ctx.expected_balances:
        logger.info(
            "Balances caught up, took %s blocks, moving on", height - sent_height
        )
        ctx.send_moar_txs(hash_, 10, use_routing=True)
        sent_height = height
        caught_up_times += 1
    else:
        assert height <= sent_height + 30, (
            "Balances before: {before}\n"
            "Expected balances: {expected}\n"
            "Current balances: {current}\n"
            "Sent at height: {height}"
        ).format(
