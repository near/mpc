...
def unstake_and_stake(node, tx_sender_node):
    ...
    nonce = (
        tx_sender_node.get_nonce_for_pk(node.signer_key.account_id, node.signer_key.pk)
        + 10
    )
    ...
    tx = sign_staking_tx(
        node.signer_key, node.validator_key, full_balance // 2, nonce, hash_
    )
    nonce += 10
    res = tx_sender_node.send_tx_and_wait(tx, timeout=15)
    assert "error" not in res, res
...
def main():
    ...
    logging.info("Stable node has produced blocks... Stopping the stable node... ")
    ...
    logging.info("Current node has produced blocks... Stopping the current node... ")
