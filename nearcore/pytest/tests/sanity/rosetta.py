...
class RosettaExecResult:
    ...
    def related(self, num: int) -> typing.Optional[JsonDict]:
        ...
        if len(related) <= num:
            return None
        return type(self)(
            self._rpc, block["block_identifier"], related[num]["transaction_identifier"]
        )
...
class RosettaRPC:
    ...
    def __init__(
        self, *, node: cluster.BaseNode, host: str = "127.0.0.1", port: int = 5040
    ) -> None:
        self.node = node
    ...
    def exec_operations(self, signer: key.Key, *operations) -> RosettaExecResult:
        ...
        options = self.rpc("/construction/preprocess", operations=operations)["options"]
        metadata = self.rpc(
            "/construction/metadata", options=options, public_keys=[public_key]
        )["metadata"]
        payloads = self.rpc(
    ...
    def transfer(
        self, *, src: key.Key, dst: key.Key, amount: int, **kw
    ) -> RosettaExecResult:
        ...
        return self.exec_operations(
            src,
            {
                ...
                "type": "TRANSFER",
                "account": {"address": src.account_id},
            ...
            },
            {
                ...
                "type": "TRANSFER",
                "account": {"address": dst.account_id},
                "amount": {"value": str(amount), "currency": currency},
            },
            **kw,
        )
        ...
        return self.exec_operations(
            ...
            },
            {
                ...
                "metadata": {
                    ...
                    "args": bytes(
                        json.dumps({"receiver_id": dst.account_id, "amount": 1}),
                        "utf-8",
                    ).hex(),
    ...
    def add_full_access_key(
        self, account: key.Key, public_key_hex: str, **kw
    ) -> RosettaExecResult:
        return self.exec_operations(
            account,
            {
                ...
                "account": {"address": account.account_id},
            },
            {
                "operation_identifier": {"index": 1},
                "related_operations": [{"index": 0}],
    ...
    def delete_account(
        self, account: key.Key, refund_to: key.Key, **kw
    ) -> RosettaExecResult:
        return self.exec_operations(
            account,
            {
            ...
            },
            {
                ...
                "account": {"address": refund_to.account_id},
            },
            **kw,
        )
    ...
    def get_transaction(
        self, *, block_id: BlockIdentifier, tx_id: TxIdentifier
    ) -> JsonDict:
        res = self.rpc(
            "/block/transaction",
    ...
    def get_account_balances(self, *, account_id: str) -> JsonDict:
        res = self.rpc(
            "/account/balance", account_identifier=account_identifier(account_id)
        )
        return res["balances"]
...
class RosettaTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.node = cluster.start_cluster(
            1,
            0,
            1,
            {},
    ...
    def test_zero_balance_account(self) -> None:
        ...
        self.assertEqual(
            balances, [{"value": "1", "currency": {"symbol": "NEAR", "decimals": 24}}]
        )

        # transfer the rest of the amount
        result = self.rosetta.transfer(
            src=validator, dst=implicit, amount=(test_amount - 1)
        )
        ...
        self.assertEqual(
            test_amount - key_space_cost * len(public_keys_hex),
    ...
    def test_get_block(self) -> None:
        ...
        trans_0 = {
            ...
            "operations": [
                {
                    "account": {"address": "near"},
                    "amount": {
                        "currency": {"decimals": 24, "symbol": "NEAR"},
            ...
            "transaction_identifier": {"hash": trans_0_id},
        }
        self.assertEqual(
            {
        ...
        self.assertEqual(
            {
                "block_identifier": {"hash": block_1_id["hash"], "index": 1},
                "parent_block_identifier": block_0_id,
        ...
        self.assertEqual(
            trans_1, self.rosetta.get_transaction(block_id=block_1_id, tx_id=trans_1_id)
        )
    ...
    def test_fungible_token_transfer(self) -> None:
        ...
        deploy_contract_response = self.node.send_tx_and_wait(deploy_contract_tx, 100)
        ...
        result = self.rosetta.transfer(src=validator, dst=implicit, amount=test_amount)
        ### 3. Storage deposit
        ...
        ### 4. Rosetta ft_call
        ft_result = self.rosetta.ft_transfer(
    ...
    def test_get_block_nonexistent(self) -> None:
        ...
        test(404, self.rosetta.get_transaction, block_id=block_0_id, tx_id=trans_1_id)
        test(404, self.rosetta.get_transaction, block_id=block_1_id, tx_id=trans_0_id)

    def _get_account_balance(
        self, account: key.Key, require: bool = True
    ) -> typing.Optional[int]:
                ...
                "operations": [
                    {
                        ...
                        "type": "TRANSFER",
                        "metadata": {
                            "predecessor_id": {"address": implicit.account_id}
                        },
                    }
                ],
                "related_transactions": [
        ...
        self.assertEqual(
            {
                ...
                "related_transactions": [
                    {"direction": "forward", "transaction_identifier": receipt_id_2}
                ],
                "transaction_identifier": receipt_id,
            },
            result.transaction(),
