...
near_root, node_dirs = init_cluster(
    ...
    {0: node_config_dump, 1: node_config_sync},
)
...
def random_workload_until(target, nonce, keys, target_node):
    ...
    while True:
        ...
        if (len(keys) > 100 and random.random() < 0.2) or len(keys) > 1000:
            ...
            call_function("read", key, nonce, boot_node.signer_key, last_block_hash)
            ...
            payment_tx = transaction.sign_payment_tx(
                key_from, account_to, 1, nonce, last_block_hash
            )
            boot_node.send_tx(payment_tx).get("result")
        else:
            key = random_u64()
            keys.append(key)
            call_function("write", key, nonce, boot_node.signer_key, last_block_hash)
            call_function("write", key, nonce, node1.signer_key, last_block_hash)
    return (nonce, keys)
...
nonce, keys = random_workload_until(int(EPOCH_LENGTH * 2.7), nonce, keys, boot_node)
...
nonce, keys = random_workload_until(int(EPOCH_LENGTH * 3.9), nonce, keys, boot_node)
