...
class TestRpcViewHistory(unittest.TestCase):
    def test_rpc_view_history(self):
        ...
        config = {
            node_id: {"consensus": consensus, "store": store} for node_id in range(3)
        }

        nodes: List[LocalNode] = start_cluster(
            3,
            0,
            1,
            None,
        ...
        tx = sign_payment_tx(
            nodes[0].signer_key, "test1", token_transfer, 1, latest_block_hash
        )
        ...
        for acc_id in ["test0", "test1"]:
            ...
            amount_delta = (
                int(nodes[0].get_account(acc_id, "final")["result"]["amount"])
                - balances[acc_id]
            )
            ...
            historical_amount = int(
                nodes[0].get_account(acc_id, block=1)["result"]["amount"]
            )
