...
if __name__ == "__main__":
    ...
    node0_config = {
        "consensus": {
            "min_block_production_delay": {"secs": 0, "nanos": 1000000000},
        },
    }
    node1_config = {
        "consensus": {
            "header_sync_initial_timeout": {"secs": 3, "nanos": 0},
            "header_sync_stall_ban_timeout": {"secs": 5, "nanos": 0},
        },
    ...
    while True:
        ...
        if should_ban:
            ...
            if (
                abs(node1_height - cur_height) < 5
                and status1["sync_info"]["syncing"] is False
            ):
                break
        time.sleep(2)

    if not should_ban and (
        tracker0.check_re(BAN_STRING) or tracker1.check_re(BAN_STRING)
    ):
