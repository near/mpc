    ...
    async def handle(self, msg, fr, to):
        if msg.enum == "Routed":
            ...
            if msg_kind == "VersionedPartialEncodedChunk":
                inner_header = (
                    msg.Routed.body.VersionedPartialEncodedChunk.inner_header()
                )
            ...
            if msg_kind == "PartialEncodedChunkRequest":
                if fr == 4:
                    ...
                    assert hash_ in self.hash_to_metadata, (
                        "chunk hash %s is not present" % base58.b58encode(hash_)
                    )
...
if __name__ == "__main__":
    ...
    archival_node_config_changes = {
        ...
        "network": {"ttl_account_id_router": {"secs": 1, "nanos": 0}},
    }
    ...
    node1 = spin_up_node(
        config, near_root, node_dirs[1], 1, boot_node=boot_node, proxy=proxy
    )
    ...
    utils.wait_for_blocks(
        boot_node, target=HEIGHTS_BEFORE_ROTATE, timeout=timeout.left_seconds()
    )

    node2 = spin_up_node(
        config, near_root, node_dirs[2], 2, boot_node=boot_node, proxy=proxy
    )
    node3 = spin_up_node(
        config, near_root, node_dirs[3], 3, boot_node=boot_node, proxy=proxy
    )
    ...
    while True:
        if (
            not node2.get_status()["sync_info"]["syncing"]
    ...
    for stake, nodes, expected_vals in [
        ...
        for ord_, node in enumerate(reversed(nodes)):
            tx = sign_staking_tx(node.signer_key, node.validator_key, stake, 10, hash_)
    ...
    height_to_sync_to, _ = utils.wait_for_blocks(
        node2, target=target, timeout=timeout.left_seconds()
    )
    ...
    logging.info(
        "Waiting for the new node to sync.  "
        ...
        for shard in (0, 1):
            for node in (2, 3):
                ...
                if (height, shard, node) in requests:
                    assert (height, shard, node) in responses, (height, shard, node)
                    assert (height, shard, other) not in requests, (
                        height,
                        shard,
                        other,
                    )
                    assert (height, shard, other) not in responses, (
                        height,
                        shard,
                        other,
                    )
