...
partial_execution_outcome_schema = dict(
    [
        ...
        [Unknown, {"kind": "struct", "fields": []}],
        [Failure, {"kind": "struct", "fields": []}],
    ]
)
...
def check_transaction_outcome_proof(nodes, should_succeed, nonce):
    ...
    for cur_height, hash_ in utils.poll_blocks(nodes[0]):
        if (
            cur_height > latest_block_height + 2
            and light_client_request_block_hash is None
        ):
    ...
    light_client_block = nodes[0].json_rpc(
        "next_light_client_block", [light_client_request_block_hash]
    )["result"]
    light_client_block_hash = compute_block_hash(
        light_client_block["inner_lite"],
        light_client_block["inner_rest_hash"],
        light_client_block["prev_block_hash"],
    ).decode("utf-8")

    queries = [
        {
            "type": "transaction",
            "transaction_hash": function_call_result["result"]["transaction_outcome"][
                "id"
            ],
            "sender_id": "test0",
    ...
    for query, (outcome, id) in zip(queries, outcomes):
        ...
        execution_outcome_hash = hashlib.sha256(
            serialize_execution_outcome_with_id(outcome, id)
        ).digest()
        outcome_root = utils.compute_merkle_root_from_path(
            light_client_proof["outcome_proof"]["proof"], execution_outcome_hash
        )
        ...
        block = nodes[0].json_rpc(
            "block", {"block_id": light_client_proof["outcome_proof"]["block_hash"]}
        )
        expected_root = block["result"]["header"]["outcome_root"]
        assert base58.b58decode(expected_root) == block_outcome_root, (
            f"expected outcome root {expected_root} actual {base58.b58encode(block_outcome_root)}"
        )
        ...
        assert light_client_proof["outcome_proof"][
            "block_hash"
        ] == computed_block_hash.decode("utf-8"), (
        ...
        assert (
            base58.b58decode(light_client_block["inner_lite"]["block_merkle_root"])
            == block_merkle_root
        ), (
            f"expected block merkle root {light_client_block['inner_lite']['block_merkle_root']} actual {base58.b58encode(block_merkle_root)}"
        )
