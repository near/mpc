...
def submit_tx_and_check(nodes, node_index, tx):
    ...
    res = node.send_tx_and_wait_until(tx, wait_until="EXECUTED_OPTIMISTIC", timeout=20)
    ...
    is_local_receipt = (
        res["result"]["transaction"]["signer_id"]
...
def test_tx_status(nodes, *, nonce_offset: int = 0):
    ...
    payment_tx = transaction.sign_payment_tx(
        signer_key, "test1", 100, nonce_offset + 1, encoded_block_hash
    )
    ...
    function_call_receipts = [
        r
        for r in res["result"]["receipts"]
        if "Action" in r["receipt"]
        and any(
            "FunctionCall" in action for action in r["receipt"]["Action"]["actions"]
        )
    ]
    assert len(function_call_receipts) == 1, function_call_receipts
    assert (
        function_call_receipts[0]["receipt"]["Action"]["actions"][0]["FunctionCall"][
            "method_name"
        ]
        == "ext_sha256"
    ), function_call_receipts
