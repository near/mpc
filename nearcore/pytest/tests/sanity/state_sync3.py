...
node0_height, _ = utils.wait_for_blocks(
    nodes[0], target=EPOCH_LENGTH * 2 + 1, poll_interval=5
)
...
for node1_height, _ in utils.poll_blocks(
    nodes[1], timeout=MAX_SYNC_WAIT, poll_interval=2
):
    ...
    if node1_height >= EPOCH_LENGTH:
        if state_sync_done_time is None:
            ...
            assert node1_height > state_sync_done_height, (
                "No progress after state sync is done"
            )
