...
res = nodes[0].json_rpc("broadcast_tx_async", [base64.b64encode(tx).decode("utf8")])
assert "error" not in res, res
time.sleep(5)
...
tx = sign_payment_tx(
    nodes[0].signer_key, "test1", 100, 10, base58.b58decode(hash_.encode("utf8"))
)
# check a transaction that doesn't exist yet
params = {"signed_tx_base64": base64.b64encode(tx).decode("utf8"), "wait_until": "NONE"}
