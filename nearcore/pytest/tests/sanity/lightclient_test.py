...
no_state_snapshots_config["store.state_snapshot_config.state_snapshot_type"] = (
    "Disabled"
)
...
def get_light_client_block(hash_, last_known_block):
    ...
    if ret["result"] != {} and last_known_block is not None:
        validate_light_client_block(
            last_known_block, ret["result"], block_producers_map, panic=True
        )
    return ret
...
def get_up_to(from_, to):
    ...
    for height, hash_ in utils.poll_blocks(
        nodes[0], timeout=TIMEOUT, poll_interval=0.01
    ):
        ...
        if (
            first_epoch_switch_height is None
...
heights = [
    None,
    3 + first_epoch_switch_height,
    9 + first_epoch_switch_height,
    15 + first_epoch_switch_height,
...
while True:
    ...
    assert res["result"]["inner_lite"]["height"] == heights[iter_], (
        res["result"]["inner_lite"],
        first_epoch_switch_height,
    )

    last_known_block_hash = compute_block_hash(
        res["result"]["inner_lite"],
        res["result"]["inner_rest_hash"],
        res["result"]["prev_block_hash"],
    ).decode("ascii")
    assert (
        last_known_block_hash == height_to_hash[res["result"]["inner_lite"]["height"]]
    ), "%s != %s" % (
    ...
    if last_known_block is None:
        block_producers_map[res["result"]["inner_lite"]["next_epoch_id"]] = res[
            "result"
        ]["next_bps"]
...
for i in range(2):
    ...
    assert res["result"]["inner_lite"]["height"] == 21 + first_epoch_switch_height, (
    ...
    get_up_to(i + 25 + first_epoch_switch_height, i + 25 + first_epoch_switch_height)
