...
def check_account_status(node: LocalNode, account_id: str) -> typing.Tuple[int, int]:
...
class TestMetaTransactions(unittest.TestCase):
    def test_meta_tx(self):
        ...
        tx = transaction.sign_create_account_with_full_access_key_and_balance_tx(
            ...
            candidate_key,
            CANDIDATE_STARTING_AMOUNT,
            node0_nonce.use_nonce(),
            base58.b58decode(hash_.encode("utf8")),
        )
        nodes[0].send_tx_and_wait(tx, 100)

        self.assertEqual(
            check_account_status(nodes[0], CANDIDATE_ACCOUNT),
            (1, CANDIDATE_STARTING_AMOUNT),
        )

        candidate_nonce = create_nonce_from_node(
            nodes[0], candidate_key.account_id, candidate_key.pk
        )
        ...
        signed_meta_tx = transaction.create_signed_delegated_action(
            ...
            CANDIDATE_ACCOUNT,
            [add_new_key_action],
            candidate_nonce.use_nonce(),
            1000,
            candidate_key.decoded_pk(),
