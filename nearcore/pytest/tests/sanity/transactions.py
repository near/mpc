...
tx = sign_payment_tx(
    nodes[0].signer_key, "test1", 100, 1, base58.b58decode(hash.encode("utf8"))
)
nodes[4].send_tx(tx)
...
height, hash = utils.wait_for_blocks(
    nodes[4], target=sent_height + 6, check_storage=False
)
...
for height, hash in utils.poll_blocks(nodes[4], timeout=TIMEOUT):
    if ctx.get_balances() == ctx.expected_balances:
        ...
        assert height <= sent_height + 10, (
            "Balances before: {before}\n"
