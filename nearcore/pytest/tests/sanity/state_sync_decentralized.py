...
class StateSyncValidatorShardSwap(unittest.TestCase):
    def _prepare_cluster(self):
        ...
        configs[NUM_VALIDATORS]["store.state_snapshot_config.state_snapshot_type"] = (
            "Disabled"
        )
        ...
        for i in range(len(self.nodes) - 1):
            ...
            for key, value in down:
                ...
                if key["source"] != "network":
                    assert False, (
                        f"Expected success only from 'network', got {key['source']}"
                    )
                ...
                if key["type"] == "header":
                    ...
                    assert False, (
                        f"Unexpected near_state_sync_download_result value ({key}, {value})"
                    )
            ...
            assert num_headers > 0 and num_parts > 0, (
                f"Node {i} did not state sync, but is expected to in this test"
            )
