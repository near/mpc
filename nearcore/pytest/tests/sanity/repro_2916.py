    ...
    for step in range(2):
        ...
        for chunk in chunks:
            ...
            def predicate(response):
                return (
                    response.enum == "Routed"
                    and response.Routed.body.enum == "PartialEncodedChunkResponse"
                )
            ...
            try:
                ...
                assert False, (
                    "A response is always expected for partial encoded chunk request."
                )

            for (
                receipt_proof
            ) in response.Routed.body.PartialEncodedChunkResponse.receipts:
                shard_proof = receipt_proof.f2
                assert shard_proof.from_shard_id == shard_id, (
                    "Basic correctness check failed: the receipt for chunk of shard {} has the wrong from_shard_id {}".format(
                        shard_id, shard_proof.from_shard_id
                    )
                )
            ...
            if tracking_shards_scenario == [0, 1]:
                if shard_id == 0:
                    assert received_receipt_shards == set([0, 1]), (
                        "Request to node 0 (tracks shard 0), chunk 0, expected receipts to [0, 1], actual {}".format(
                            received_receipt_shards
                        )
                    )
                else:
                    assert received_receipt_shards == set([0]), (
                        "Request to node 0 (tracks shard 0), chunk 1, expected receipts to [0], actual {}".format(
                            received_receipt_shards
                        )
                    )
            else:  # [1, 0]
                if shard_id == 0:
                    assert received_receipt_shards == set([1]), (
                        "Request to node 0 (tracks shard 1), chunk 0, expected receipts to [1], actual {}".format(
                            received_receipt_shards
                        )
                    )
                else:
                    assert received_receipt_shards == set([0, 1]), (
                        "Request to node 0 (tracks shard 1), chunk 1, expected receipts to [0, 1], actual {}".format(
                            received_receipt_shards
                        )
                    )
