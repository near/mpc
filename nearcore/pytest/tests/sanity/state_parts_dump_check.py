...
def main():
    ...
    client_config_changes = {0: node_config, 1: node_config_dump, 2: node_config}
    ...
    dump_node = spin_up_node(config, near_root, node_dirs[1], 1, boot_node=boot_node)
    dump_check = spin_up_node(config, near_root, node_dirs[2], 2, boot_node=boot_node)
    ...
    dump_folder = node_config_dump["state_sync"]["dump"]["location"]["Filesystem"][
        "root_dir"
    ]
    ...
    list(takewhile(lambda b: b.height < EPOCH_LENGTH - 5, poll_blocks(boot_node)))
    ...
    assert (
        ...
        == NUM_SHARDS
    ), f"Dumper process missing for some shards. {metrics}"
    ...
    list(
        takewhile(
            lambda height: height < 2, poll_epochs(boot_node, epoch_length=EPOCH_LENGTH)
        )
    )
    list(islice(poll_blocks(boot_node), 10))
    ...
    assert (
        ...
        >= NUM_SHARDS
    ), f"Some parts are missing. {metrics}"
    assert (
        sum(
            [
                val
        ...
        == NUM_SHARDS
    ), f"Some headers are missing. {metrics}"
