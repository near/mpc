...
def get_stakes():
    return [
        int(nodes[2].get_account("test%s" % i)["result"]["locked"]) for i in range(3)
    ]
...
def do_moar_stakes(last_block_hash, update_expected):
    ...
    if len(sequence) == 0:
        ...
        stakes[random.randint(0, 2)] = (
            random.randint(70000000000000000000000000, 100000000000000000000000000)
            * 1000000
        )
        stakes[random.randint(0, 2)] = (
            random.randint(70000000000000000000000000, 100000000000000000000000000)
    ...
    logger.info(
        "Sent %s staking txs: %s" % ("REAL" if update_expected else "fake", stakes)
    )
...
def doit(seq=[]):
    ...
    nodes = start_cluster(
        ...
        ],
        {
            0: {
                ...
                "consensus": {
                    "state_sync_external_timeout": {"secs": 2, "nanos": 0},
                    "state_sync_p2p_timeout": {"secs": 2, "nanos": 0},
                    "state_sync_external_backoff": {"secs": 2, "nanos": 0},
                },
            },
            1: {
                "tracked_shards_config": "AllShards",
    ...
    while True:
        ...
        if (height + EPOCH_LENGTH - FAKE_OFFSET) // EPOCH_LENGTH > (
            last_fake_stakes_height + EPOCH_LENGTH - FAKE_OFFSET
        ) // EPOCH_LENGTH:
            last_iter = time.time()
        ...
        if (height + EPOCH_LENGTH - REAL_OFFSET) // EPOCH_LENGTH > (
            last_staked_height + EPOCH_LENGTH - REAL_OFFSET
        ) // EPOCH_LENGTH:
            send_reals = True
