...
class Cluster:
    ...
    def start_node(
        self, ordinal: int, *, boot_node: typing.Optional[cluster.BaseNode]
    ) -> cluster.BaseNode:
...
def get_metrics(node_name: str, node: cluster.BootNode) -> typing.Dict[str, int]:
    ...
    counts = dict(
        (sample.labels["method"] + "/" + sample.labels["success"], int(sample.value))
        for sample in histogram.samples
        if sample.name.endswith("_count")
    )
    logger.info(
        f"{node_name} counters: "
...
def assert_metrics(
    metrics: typing.Dict[str, int], allowed_non_zero: typing.Sequence[str]
) -> None:
...
def run_test(cluster: Cluster) -> None:
    ...
    utils.wait_for_blocks(barney, target=fred_blocks[-1].height, poll_interval=1)
