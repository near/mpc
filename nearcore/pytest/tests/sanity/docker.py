...
def run(cmd: _Command, *, capture_output: bool = False) -> typing.Optional[str]:
    ...
    if capture_output:
        return subprocess.check_output(cmd, cwd=_REPO_DIR, encoding="utf-8").strip()
...
def docker_run(
    ...
    *,
    ...
    rust_log = (
        "actix_web=warn,mio=warn,tokio_util=warn,"
        "actix_server=warn,actix_http=warn," + os.environ.get("RUST_LOG", "debug")
    )
...
class DockerNode(cluster.LocalNode):
    ...
    def start(self, *, boot_node: cluster.BootNode = None) -> None:
        ...
        cid = docker_run(
            detach=True, network=True, volume=(self.node_dir, "/srv/near"), env=env
        )
    ...
    def output_logs(self):
        ...
        if self.__WARN_LOGS:
            logger.info("Due to technical limitations logs from node is not available")
