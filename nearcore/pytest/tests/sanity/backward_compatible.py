...
def main():
    ...
    stable_node = cluster.spin_up_node(
        config, executables.stable.root, str(node_root / "test0"), 0
    )
    config = executables.current.node_config()
    current_node = cluster.spin_up_node(
        config,
    ...
    new_signer_key = cluster.Key(
        new_account_id, stable_node.signer_key.pk, stable_node.signer_key.sk
    )
    create_account_tx = sign_create_account_with_full_access_key_and_balance_tx(
        stable_node.signer_key, new_account_id, new_signer_key, 10**24, 1, block_hash
    )
    res = stable_node.send_tx_and_wait(create_account_tx, timeout=20)
    assert "error" not in res, res
    assert "Failure" not in res["result"]["status"], res

    transfer_tx = sign_payment_tx(
        stable_node.signer_key, new_account_id, 10**25, 2, block_hash
    )
    ...
    test_contract = utils.load_test_contract(config=executables.current.node_config())
    tx = sign_deploy_contract_tx(new_signer_key, test_contract, nonce, block_hash)
    res = stable_node.send_tx_and_wait(tx, timeout=20)
    assert "error" not in res, res

    tx = sign_deploy_contract_tx(stable_node.signer_key, test_contract, 3, block_hash)
    res = stable_node.send_tx_and_wait(tx, timeout=20)
    assert "error" not in res, res
