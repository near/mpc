    ...
    while True:
        ...
        if random.random() < 0.5:
            ...
            for node in [node0, node1]:
                call_function(
                    "read", key, nonce, node.signer_key, last_block_hash, node0
                )
                call_function(
                    "read", key, nonce, node.signer_key, last_block_hash, node0
                )
            ...
            for node in [node0, node1]:
                call_function(
                    "write", key, nonce, node.signer_key, last_block_hash, node0
                )
    return nonce, keys
...
def main():
    ...
    nonce, keys = random_workload_until(
        EPOCH_LENGTH - 5, 1, [], boot_node, node1, boot_node
    )
    ...
    nonce, keys = random_workload_until(
        int(EPOCH_LENGTH * 3), nonce, keys, boot_node, node1, boot_node
    )
    ...
    nonce, keys = random_workload_until(
        int(EPOCH_LENGTH * 3.7), nonce, keys, boot_node, node1, node1
    )
