...
def test_changes_with_new_account_with_access_key():
    ...
    for request in [
        ...
        assert_changes_response(
            request=request, expected_response={"block_hash": block_hash, "changes": []}
        )

    # Test happy-path
    block_header = nodes[0].get_block(block_hash)["result"]["header"]
    prev_block_header = nodes[0].get_block(block_header["prev_hash"])["result"][
        "header"
    ]
        ...
        "changes": [
            {
                "cause": {
                    ...
                    "receipt_hash": new_account_response["result"]["receipts_outcome"][
                        0
                    ]["id"],
                },
                ...
                "change": {
                    ...
                    "access_key": {"nonce": nonce, "permission": "FullAccess"},
                },
            }
        ],
    }
    for request in [
        ...
        },
        {
            ...
            "changes_type": "all_access_key_changes",
            "account_ids": [
                new_key.account_id + "_non_existing1",
                new_key.account_id,
        ...
        "changes": [
            {
                "cause": {
                    ...
                    "tx_hash": delete_access_key_response["result"]["transaction"][
                        "hash"
                    ],
                },
                "type": "access_key_update",
            ...
            },
            {
                "cause": {
                    "type": "receipt_processing",
                    "receipt_hash": delete_access_key_response["result"][
                        "receipts_outcome"
                    ][0]["id"],
                },
...
def test_key_value_changes():
        ...
        "changes": [
            {
                "cause": {
                    "type": "receipt_processing",
                    "receipt_hash": deploy_contract_response["result"][
                        "receipts_outcome"
                    ][0]["id"],
                },
    ...
    def set_value(value, *, nonce):
        ...
        try:
            status = response["result"]["receipts_outcome"][0]["outcome"]["status"]
    ...
    for request in [
        ...
        # Test non-existing key prefix
        {
            **base_request,
            ...
            "key_prefix_base64": base64.b64encode(struct.pack("<Q", 24)).decode(
                "ascii"
            ),
        },
    ]:
        ...
        "changes": [
            {
                ...
                "change": {
                    ...
                    "value_base64": base64.b64encode(struct.pack("<Q", 10)).decode(
                        "ascii"
                    ),
                },
            },
            {
                ...
                "change": {
                    "account_id": contract_key.account_id,
                    "key_base64": key_base64,
                    "value_base64": base64.b64encode(struct.pack("<Q", 20)).decode(
                        "ascii"
                    ),
                },
            },
