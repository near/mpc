...
nodes = start_cluster(
    ...
    {i: node_config for i in range(8)},
)
...
(kickouts, current_validators, _next_validators) = get_epoch_info(rpc_node, hash)
for i in range(4):
    assert_block_and_chunk_producer(find_account(current_validators, f"test{i}"))
for i in range(4, 8):
...
(kickouts, _current_validators, next_validators) = get_epoch_info(rpc_node, hash)
...
assert find_account(next_validators, "test3") is None, (
    "test3 must not be in next validators"
)
assert find_account(next_validators, "test7") is None, (
    "test7 must not be in next validators"
)
