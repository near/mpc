...
from transaction import (
    sign_deploy_global_contract_tx,
    sign_function_call_tx,
    sign_use_global_contract_tx,
)
...
def test_deploy_global_contract():
    ...
    val_client_config_changes = {
        i: {"tracked_shards_config": "NoShards"} for i in range(n)
    }
...
def deploy_global_contract(node, contract, deploy_mode, nonce):
    last_block_hash = node.get_latest_block().hash_bytes
    tx = sign_deploy_global_contract_tx(
        node.signer_key, contract, deploy_mode, nonce, last_block_hash
    )
    node.send_tx(tx)
    time.sleep(3)
...
def use_global_contract(node, identifier, nonce):
    last_block_hash = node.get_latest_block().hash_bytes
    tx = sign_use_global_contract_tx(
        node.signer_key, identifier, nonce, last_block_hash
    )
    node.send_tx(tx)
    time.sleep(3)
