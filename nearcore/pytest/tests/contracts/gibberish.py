...
nodes = start_cluster(
    ...
    {0: node_config, 1: node_config, 2: node_config},
)
...
for iter_ in range(10):
    ...
    wasm_blob = wasm_blob_1.replace(
        bytes("something", "utf8"), bytes(new_name, "utf8")
    ).replace(bytes("hello", "utf8"), bytes(new_output, "utf8"))
