...
class Follow(SocialDbSet):
    def __init__(
        self, contract_id: str, sender: Account, follow_list: typing.List[str]
    ):
        super().__init__(contract_id, sender)
...
class InitSocialDbAccount(FunctionCall):
    ...
    def __init__(self, contract_id: str, account: Account):
        super().__init__(account, contract_id, "storage_deposit", balance=1 * NEAR_BASE)
        self.contract_id = contract_id
...
def social_db_build_index_obj(key_list_pairs: dict) -> dict:
    ...
    return {key: serialize_values(values) for key, values in key_list_pairs.items()}
...
def on_locust_init(environment, **kwargs):
    ...
    environment.social_account_id = (
        f"social{environment.parsed_options.run_id}.{funding_account.key.account_id}"
    )
    ...
    if not isinstance(environment.runner, runners.WorkerRunner):
        ...
        existed = node.prepare_account(
            social_account, funding_account, 50000, "create contract account"
        )
        if not existed:
            node.send_tx_retry(
            ...
            node.send_tx_retry(InitSocialDB(social_account), "init socialDB contract")
