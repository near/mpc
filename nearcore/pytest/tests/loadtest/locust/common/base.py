...
class Account:
    ...
    def fast_forward_nonce(self, ak_nonce):
        with self.current_nonce.get_lock():
            self.current_nonce.value = max(self.current_nonce.value, ak_nonce + 1)
...
class FunctionCall(Transaction):
    def __init__(
        self, sender: Account, receiver_id: str, method: str, balance: int = 0
    ):
        super().__init__()
        self.sender = sender
    ...
    def sign(self, block_hash) -> transaction.SignedTransaction:
        return transaction.sign_function_call_transaction(
            ...
            self.receiver_id,
            self.method,
            json.dumps(self.args()).encode("utf-8"),
            self.attached_gas(),
            self.balance,
    ...
    def sign(self, block_hash) -> transaction.SignedTransaction:
        ...
        def create_action(args):
            return transaction.create_function_call_action(
                self.method, json.dumps(args).encode("utf-8"), gas, int(self.balance)
            )
...
class AddFullAccessKey(Transaction):
    ...
    def sign(self, block_hash) -> transaction.SignedTransaction:
        action = transaction.create_full_access_key_action(self.new_key.decoded_pk())
        return transaction.sign_transaction(
            self.sender.key.account_id,
            self.sender.use_nonce(),
            [action],
...
class NearNodeProxy:
        ...
        try:
            try:
                ...
                result = self.post_json(
                    "send_tx",
                    {
                        "signed_tx_base64": base64.b64encode(serialized_tx).decode(
                            "utf8"
                        ),
                ...
                if not "result" in submit_response:
                    meta["exception"] = RpcError(
                        message="Failed to submit transaction", details=submit_response
                    )
    ...
    def send_tx_async(self, tx: Transaction, locust_name: str) -> dict:
        ...
        try:
            submit_raw_response = self.post_json(
                "send_tx",
                {
                    "signed_tx_base64": base64.b64encode(serialized_tx).decode("utf8"),
                    "wait_until": "NONE",
                },
            )
            meta["response_length"] = len(submit_raw_response.text)
    ...
    def final_block_hash(self):
        return base58.b58decode(self.node.get_final_block()["result"]["header"]["hash"])
    ...
    def post_json(self, method: str, params: typing.Dict[str, str]):
        ...
        try:
            ...
            session = Session(
                connection_timeout=6, network_timeout=9, max_retries=5, retry_delay=0.1
            )
            return session.post(url="http://%s:%s" % self.node.rpc_addr(), json=j)
    ...
    def poll_tx_result(self, meta: dict, tx: Transaction):
        ...
        try:
            meta["response"] = evaluate_rpc_result(json.loads(result_response.text))
        except:
    ...
    def prepare_account(
        self, account: Account, parent: Account, balance: float, msg: str
    ) -> bool:
        """
    ...
    def prepare_accounts(
        ...
        while len(to_create) + len(to_refresh) + len(inflight) > 0:
            ...
            for tx, meta, account in inflight:
                ...
                self.poll_tx_result(meta, tx)
                meta["response_time"] = (time.time() - meta["start_time"]) * 1000
                to_refresh.append(account)
                # Locust tracking
                self.request_event.fire(**meta)
            inflight.clear()
...
class NearUser(User):
    ...
    def send_tx_async(self, tx: Transaction, locust_name="generic send_tx_async"):
        """
        Send a transaction, but don't wait until it completes.
        """
        return self.node.send_tx_async(tx, locust_name)["response"]

    def send_tx_retry(self, tx: Transaction, locust_name="generic send_tx_retry"):
        """
        Send a transaction and retry until it succeeds
        """
...
class InvalidNonceError(RpcError):
    def __init__(
        ...
        super().__init__(
            ...
            details=f"Tried to use nonce {used_nonce} but access key nonce is {ak_nonce}",
        )
...
class ShardCongestedError(RpcError):
    def __init__(
        ...
        super().__init__(
            ...
            details=f"Shard {shard_id} is currently at congestion level {congestion_level} and rejects new transactions",
        )
...
class ShardStuckError(RpcError):
    def __init__(
        ...
        super().__init__(
            message="Shard stuck",
            details=f"Shard {shard_id} missed {missed_chunks} chunks and rejects new transactions",
        )
        self.shard_id = shard_id
...
class FunctionExecutionError(ReceiptError):
    def __init__(
        self, status, receipt_id, message="Smart contract function execution failed"
    ):
        super().__init__(status, receipt_id, message)
...
def evaluate_rpc_result(rpc_result):
    ...
    for receipt in receipt_outcomes:
        ...
        if status == "Unknown":
            raise ReceiptError(
                receipt["outcome"], receipt["id"], message="Unknown receipt result"
            )
        if status == "Failure":
            failure = receipt["outcome"]["status"]["Failure"]
            panic_msg = as_smart_contract_panic_message(failure)
            if panic_msg:
                raise SmartContractPanic(
                    receipt["outcome"], receipt["id"], message=panic_msg
                )
            exec_failed = as_execution_error(failure)
            if exec_failed:
                raise FunctionExecutionError(
                    receipt["outcome"], receipt["id"], message=exec_failed
                )
            raise ReceiptError(receipt["outcome"], receipt["id"])
        if not status in ["SuccessReceiptId", "SuccessValue"]:
            raise ReceiptError(
                receipt["outcome"], receipt["id"], message="Unexpected status"
            )
...
def do_on_locust_init(environment):
    ...
    master_funding_key = key.Key.from_json_file(environment.parsed_options.funding_key)
    ...
    environment.account_generator = init_account_generator(environment.parsed_options)
    ...
    if isinstance(environment.runner, runners.MasterRunner):
        ...
        for index in range(num_funding_accounts):
            ...
            node.prepare_account(
                account,
                master_funding_account,
                funding_balance,
        ...
        worker_account_id = (
            f"funds_worker_{worker_id}.{master_funding_account.key.account_id}"
        )
...
def _(parser):
    ...
    parser.add_argument(
        ...
        required=False,
        help="chain ID whose shard layout we should consult when generating account IDs. Convenience option to avoid using --shard-layout-file for mainnet and testnet",
    )
    parser.add_argument(
        "--shard-layout-version",
        required=False,
        help="Version of the shard layout. Only works with --shard-layout-chain-id. Should be one of V0, V1, V2, or V3.",
    )
    parser.add_argument(
        "--run-id",
        default="",
...
class TestEvaluateRpcResult(unittest.TestCase):
    ...
    def test_smart_contract_panic(self):
        ...
        self.assertRaises(SmartContractPanic, evaluate_rpc_result, json.loads(input))
