...
class FTContract:
    ...
    def install(self, node: NearNodeProxy, parent: Account):
        ...
        existed = node.prepare_account(
            self.account, parent, FTContract.INIT_BALANCE, "create contract account"
        )
        if not existed:
    ...
    def register_user(self, user: NearUser):
        user.send_tx_retry(
            InitFTAccount(self.account, user.account), locust_name="Init FT Account"
        )
        user.send_tx_retry(
            TransferFT(
                self.account, self.ft_distributor, user.account_id, how_much=10**8
            ),
    ...
    def register_passive_user(self, node: NearNodeProxy, account: Account):
        ...
        """
        node.send_tx_async(
            InitFTAccount(self.account, account), locust_name="Init FT Account"
        )
    ...
    def random_receivers(self, sender: str, num) -> typing.List[str]:
        ...
        return list(
            map(
                lambda a: a.replace(sender, self.ft_distributor.key.account_id),
                receivers,
            )
        )

    def create_passive_users(
        self, num: int, node: NearNodeProxy, parent: Account, max_account_id_len=64
    ):
        """
        ...
        with futures.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for i in range(num_batches):
                account_ids = [
                    create_account_id(i)
                    for i in range(i * batch_size, min((i + 1) * batch_size, num))
                ]
                if create_accounts:
                    accounts = [
                    ...
                    node.prepare_accounts(
                        accounts, parent, balance=1, msg="create passive user"
                    )
                    futures.wait(
...
class TransferFT(FunctionCall):
    def __init__(self, ft: Account, sender: Account, recipient_id: str, how_much=1):
...
class InitFT(FunctionCall):
    ...
    def args(self) -> dict:
        return {"owner_id": self.contract.key.account_id, "total_supply": str(10**33)}
...
class InitFTAccount(FunctionCall):
    def __init__(self, contract: Account, account: Account):
        super().__init__(
            account, contract.key.account_id, "storage_deposit", balance=int(1e23)
        )
        self.contract = contract
        self.account = account
    ...
    for i in range(num_ft_contracts):
        ...
        if environment.parsed_options.num_passive_users > 0:
            ft_contract.create_passive_users(
                environment.parsed_options.num_passive_users, node, funding_account
            )
...
def _(parser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="How many different FT contracts to spawn from this worker (FT contracts are never shared between workers)",
    )
    parser.add_argument(
        "--fixed-contract-names",
        action="store_true",
        help="Whether the names of FT contracts will deterministically based on worker id and run id.",
    )
    parser.add_argument(
        "--num-passive-users",
        type=int,
