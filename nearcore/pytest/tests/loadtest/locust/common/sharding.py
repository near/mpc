...
def random_valid_char(lower, upper, upper_inclusive=True):
    ...
    assert (upper_inclusive and lower <= upper) or (
        not upper_inclusive and lower < upper
    )
    ...
    if lower >= "a":
        ...
        if upper <= "9":
            ...
            chars = char_range(lower, "9") + char_range("a", upper, upper_inclusive)
    elif lower == ".":
        if upper == ".":
            ...
            chars = (
                ["."] + char_range("0", "9") + char_range("a", upper, upper_inclusive)
            )
    else:
        assert lower == "-"
        if upper == "-" or upper == ".":
...
def finish_upper(lower, upper, prefix, free_chars, free_length):
    if len(upper) > 1:
        ...
        assert upper[-2] != "-" or upper[-1] != "0", (
            f"Cannot build account ID prefix less than {upper}"
        )
    else:
        assert upper[0] > "0", f"Cannot build account ID prefix less than {upper}"
...
class AccountGenerator:
    def __init__(self, available_shard_layouts, shard_layout_version):
        assert shard_layout_version in available_shard_layouts, (
            "Shard layout version not found in available versions: "
            + str(available_shard_layouts.keys())
        )
        ...
        if shard_layout_version != "V0":
            selected_shard_layout = available_shard_layouts.get(shard_layout_version)
            ...
            for i, account_id in enumerate(accounts):
                ...
                if i + 1 < len(accounts):
                    ...
                    if next_account != account_id + "-0":
                        self.shard_map[shard_offset + i + 1] = (
                            account_id,
                            next_account,
                        )
                else:
                    if account_id != "z" * 64:
                        self.shard_map[shard_offset + i + 1] = (account_id, None)
    ...
    def random_account_id(self, base_name, suffix):
        if len(self.shard_map) == 0:
            ...
            shard_id, (lower, upper) = random.choice(list(self.shard_map.items()))
            return random_account_between(base_name, suffix, lower, upper)


class TestRandomAccount(unittest.TestCase):
    ACCOUNT_REGEX = re.compile(r"^(([a-z\d]+[-_])*[a-z\d]+\.)*([a-z\d]+[-_])*[a-z\d]+$")
    ...
    def test_random_account(self):
        ...
        for lower, upper in test_cases:
            ...
            if lower is not None:
                assert TestRandomAccount.ACCOUNT_REGEX.fullmatch(lower) is not None
            if upper is not None:
                assert TestRandomAccount.ACCOUNT_REGEX.fullmatch(upper) is not None

            for base_name, suffix in (
                ("foo.near", "_ft"),
                ("mocknet-mainnet-123456789-forknet-abcdefgh.near", "_user4321_run"),
            ):
                for _ in range(10):
                    account_id = random_account_between(base_name, suffix, lower, upper)
                    assert (
                        TestRandomAccount.ACCOUNT_REGEX.fullmatch(account_id)
                        is not None
