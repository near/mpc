...
class LinkdropContract:
    ...
    def __init__(self, account: Account, linkdrop_distributor: Account, code: str):
    ...
    def install(self, node: NearNodeProxy, parent: Account):
        ...
        if not existed:
            node.send_tx_retry(
                Deploy(self.account, self.code, "LINKDROP"), "Deploy Linkdrop"
            )
    ...
    def create_drop(self, user: NearUser) -> str:
        ...
        user.send_tx(
            InitDrop(self.account, user.account, drop_id),
...
class InitDropContract(FunctionCall):
    ...
    def args(self) -> dict:
        return {
            ...
            "root_account": "near",
            "contract_metadata": {"version": "v2", "link": "google.com"},
        }
...
class InitDrop(FunctionCall):
    def __init__(self, contract: Account, account: Account, drop_id: str):
        super().__init__(
            account, contract.key.account_id, "create_drop", balance=int(1e23)
        )
        self.contract = contract
        self.account = account
...
class AddKey(FunctionCall):
    def __init__(self, linkdrop: Account, sender: Account, public_key, drop_id: str):
        super().__init__(
            sender, linkdrop.key.account_id, "add_keys", balance=int(15e22)
        )
...
class ClaimDrop(Transaction):
    ...
    def args(self) -> dict:
        return {"new_account_id": self.new_account_id, "new_public_key": self.pk}

    def sign(self, block_hash) -> transaction.SignedTransaction:
        nonce = mocknet_helpers.get_nonce_for_pk(
...
def on_locust_init(environment, **kwargs):
    ...
    for i in range(num_linkdrop_contracts):
        ...
        linkdrop_contract = LinkdropContract(
            linkdrop_account, linkdrop_account, linkdrop_contract_code
        )
...
def _(parser):
    ...
    parser.add_argument(
        ...
        required=False,
        default=5,
        help="How many different Linkdrop contracts to spawn from this worker (Linkdrop contracts are never shared between workers)",
    )
