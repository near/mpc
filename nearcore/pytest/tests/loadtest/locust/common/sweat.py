...
class SweatContract(FTContract):
    ...
    def init_contract(self, node: NearNodeProxy):
        ...
        node.send_tx_retry(
            InitFTAccount(self.account, self.ft_distributor),
    ...
    def top_up(self, node: NearNodeProxy, receiver_id: str):
        ...
        """
        node.send_tx_retry(
            SweatMint(self.account, receiver_id, 1_000_000_000_000), "top up sweat"
        )
    ...
    def register_oracle(self, node: NearNodeProxy, oracle_id: str):
        ...
        """
        node.send_tx_retry(SweatAddOracle(self.account, oracle_id), "add sweat oracle")
...
class SweatAddOracle(FunctionCall):
    ...
    def __init__(self, sweat_account: Account, oracle_id: str):
        super().__init__(sweat_account, sweat_account.key.account_id, "add_oracle")
...
class SweatGetClaimableBalanceForAccount(FunctionCall):
    ...
    def __init__(self, sweat_claim_id: str, user: Account, account_id: str):
        super().__init__(user, sweat_claim_id, "get_claimable_balance_for_account")
...
def on_locust_init(environment, **kwargs):
    ...
    sweat_account_id = (
        f"sweat{run_id}.{environment.master_funding_account.key.account_id}"
    )
    sweat_claim_account_id = (
        f"sweat-claim{run_id}.{environment.master_funding_account.key.account_id}"
    )
    oracle_account_id = worker_oracle_id(
        worker_id, run_id, environment.master_funding_account
    )

    sweat_account = Account(key.Key.from_seed_testonly(sweat_account_id))
    sweat_claim_account = Account(key.Key.from_seed_testonly(sweat_claim_account_id))
    oracle_account = Account(key.Key.from_seed_testonly(oracle_account_id))

    environment.sweat = SweatContract(
        sweat_account,
        sweat_claim_account,
        oracle_account,
    ...
    if not isinstance(environment.runner, locust.runners.WorkerRunner):
        ...
        existed = node.prepare_account(
            sweat_claim_account, funding_account, 100000, "create contract account"
        )
        if not existed:
            node.send_tx_retry(
                Deploy(
                ...
                "deploy Sweat-Claim contract",
            )
            node.send_tx_retry(
                InitClaim(sweat_claim_account, sweat_account_id),
    ...
    if isinstance(environment.runner, locust.runners.MasterRunner):
        ...
        for index in range(num_oracles):
            account_id = worker_oracle_id(
                index, run_id, environment.master_funding_account
            )
            ...
            environment.sweat.top_up(node, account_id)
            environment.sweat.register_oracle(node, account_id)
            node.send_tx_retry(
                SweatAddOracle(sweat_claim_account, account_id),
                "add sweat.claim oracle",
            )
