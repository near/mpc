...
class MintInscription(FunctionCall):
    def __init__(self, contract_account_id: str, sender: Account, tick: str, amt: int):
...
def on_locust_init(environment, **kwargs):
    ...
    node.prepare_account(account, funding_account, 50000, "create contract account")
    ...
    logger.info(f"Deploying inscription with tick {environment.inscription_tick}")
    node.send_tx_retry(
        DeployInscription(
            environment.inscription_account_id,
            account,
            environment.inscription_tick,
            100000000000,
        ),
