...
from common.sweat import (
    RecipientSteps,
    SweatGetClaimableBalanceForAccount,
    SweatMintBatch,
    SweatDeferBatch,
)
...
class SweatOracle(NearUser):
    ...
    def record_single_batch(self):
        ...
        batch_size = min(rng.randint(100, 150), len(self.sweat.registered_users))
    ...
    def on_start(self):
        ...
        self.send_tx_retry(
            AddFullAccessKey(oracle, user_oracle_key), "add user key to oracle"
        )
...
class SweatUser(NearUser):
    ...
    def get_claimable_balance_for_account(self):
        tx = SweatGetClaimableBalanceForAccount(
            self.sweat.claim.key.account_id, self.account, self.account.key.account_id
        )
