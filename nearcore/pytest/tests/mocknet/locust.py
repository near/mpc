...
have the machine used for the master process in this list as well:""")
    ...
    workers = [mocknet.get_node(instance_name) for instance_name in worker_names]
...
def upload_key(node, filename):
    node.machine.upload(
        args.funding_key, "/home/ubuntu/locust/funding_key.json", switch_user="ubuntu"
    )
...
def run_master(args, node, num_workers):
    ...
    cmd_utils.run_in_background(
        ...
        cmd,
        "locust-master.txt",
        pre_cmd="ulimit -S -n 100000 && cd /home/ubuntu/nearcore/pytest/tests/loadtest/locust",
    )
...
if __name__ == "__main__":
    ...
    run_parser.add_argument(
        "--master", type=str, required=True, help="instance name of master node"
    )
    run_parser.add_argument(
        "--workers",
        type=str,
    ...
    run_parser.add_argument(
        ...
        type=str,
        required=True,
        help="local path to a key file for the base account to be used in the test",
    )
    run_parser.add_argument(
        "--locustfile",
        type=str,
        default="ft.py",
        help="locustfile name in nearcore/pytest/tests/loadtest/locust/locustfiles",
    )
    run_parser.add_argument(
        "--max-workers",
        type=int,
    ...
    run_parser.add_argument(
        "--web-ui",
        action="store_true",
        help="if given, sets up a web UI to control the test, otherwise starts automatically",
    )
    run_parser.add_argument(
        "--num-users",
        type=int,
        help="number of users to run the test with. Required unless --web-ui is given.",
    )
    run_parser.add_argument(
        "--run-time",
        type=str,
        help="A string specifying the total run time of the test, passed to the locust --run-time argument. e.g. (300s, 20m, 3h, 1h30m, etc.)",
    )
    ...
    stop_parser.add_argument("--master", type=str, help="instance name of master node")
