...
class CommandContext:
    ...
    def get_targeted_with_schedule_ctx(self) -> list[NodeHandle]:
        ...
        """
        return [
            node.with_schedule_ctx(self.schedule_ctx) for node in self.get_targeted()
        ]
    ...
    def _get_local_nodes(self):
        ...
        """
        if (
            self.args.chain_id is not None
            or self.args.start_height is not None
            or self.args.unique_id is not None
            or self.args.mocknet_id is not None
        ):
            sys.exit(
            ...
            node_config.configure_nodes(
                nodes + to_list(traffic_generator), node_config.TEST_CONFIG
            )
    ...
    def _get_remote_nodes(self):
        ...
        if (
            self.args.chain_id is not None
            and self.args.start_height is not None
            and self.args.unique_id is not None
        ):
            mocknet_id = (
                self.args.chain_id
                + "-"
                + str(self.args.start_height)
                + "-"
                + self.args.unique_id
            )
        ...
        node_config.configure_nodes(
            nodes + to_list(traffic_generator), node_config.REMOTE_CONFIG
        )
        self.traffic_generator = traffic_generator
        self.nodes = nodes
    ...
    def _filter_hosts(self):
        ...
        if self.args.host_filter is not None:
            if self.traffic_generator is not None and not re.search(
                ...
                self.traffic_generator = None
            self.nodes = [
                h for h in self.nodes if re.search(self.args.host_filter, h.name())
            ]
...
def init_neard_runners(ctx: CommandContext, remove_home_dir=False):
    ...
    if args.neard_upgrade_binary_url is None or args.neard_upgrade_binary_url == "":
        configs = [
            {
                ...
                "binaries": [{"url": args.neard_binary_url, "epoch_height": 0}],
            }
        ] * len(nodes)
            ...
            configs.append(
                {
                    "is_traffic_generator": False,
                    "binaries": [
                        {"url": args.neard_binary_url, "epoch_height": 0},
                        {
        ...
        traffic_generator_config = {
            "is_traffic_generator": True,
            "binaries": [{"url": args.neard_upgrade_binary_url, "epoch_height": 0}],
        }

    if traffic_generator is not None:
        traffic_generator.init_neard_runner(traffic_generator_config, remove_home_dir)
    pmap(lambda x: x[0].init_neard_runner(x[1], remove_home_dir), zip(nodes, configs))
...
def get_network_nodes(new_test_rpc_responses, num_validators):
    ...
    for node, response in new_test_rpc_responses:
        if len(validators) < num_validators:
            if node.can_validate and response["validator_account_id"] is not None:
...
def new_genesis_timestamp(node):
    ...
    if err is not None:
        if err["code"] != -32601:
            sys.exit(f"bad response calling version RPC on {node.name()}: {err}")
        return None
    genesis_time = None
...
def _apply_config_changes(node, state_sync_location):
    if state_sync_location is None:
        ...
        changes = {
            "state_sync.sync": {"ExternalStorage": {"location": state_sync_location}}
        }
        if node.want_state_dump:
            ...
            changes["store.state_snapshot_config.state_snapshot_type"] = "EveryEpoch"
...
def _clear_state_parts_if_exists(location, nodes):
    ...
    state_dumper_node.run_cmd(
        f"gsutil -m rm -r gs://{bucket_name}/chain_id=*", return_on_fail=True
    )
...
def _get_state_parts_location(args):
    if args.local_test:
        return {
            "Filesystem": {
                "root_dir": str(
                    local_test_node.DEFAULT_LOCAL_MOCKNET_DIR / "state-parts"
                )
            }
        }
...
def new_test_cmd(ctx: CommandContext):
    ...
    validators, boot_nodes = get_network_nodes(
        zip(nodes, test_keys), args.num_validators
    )
    logger.info(
        """Setting validators: {0}
...
def reset_cmd(ctx: CommandContext):
    ...
    if args.backup_id is None:
        ...
        if "start" not in backups:
            backups_msg += (
                "start | None | initial test state after state root computation\n"
            )
    ...
    pmap(
        lambda node: node.neard_runner_reset(backup_id=args.backup_id),
...
def stop_nodes_cmd(ctx: CommandContext):
    pmap(lambda node: node.neard_runner_stop(), ctx.get_targeted_with_schedule_ctx())


def stop_traffic_cmd(ctx: CommandContext):
    ctx.traffic_generator.with_schedule_ctx(ctx.schedule_ctx).neard_runner_stop()


def do_update_config(node, config_change):
    result = node.neard_update_config(config_change)
    if not result:
        logger.warning(f"failed updating config on {node.name()}. result: {result}")
...
def start_nodes_cmd(ctx: CommandContext):
    ...
    if not all(pmap(lambda node: node.neard_runner_ready(), nodes)):
        ...
        return
    pmap(
        lambda node: node.with_schedule_ctx(ctx.schedule_ctx).neard_runner_start(),
        nodes,
    )
...
def start_traffic_cmd(ctx: CommandContext):
    ...
    if traffic_generator is None:
        ...
        return
    if not all(pmap(lambda node: node.neard_runner_ready(), ctx.get_targeted())):
        logger.warning(
            "not all nodes are ready to start yet. Run the `status` command to check their statuses"
        )
        return
    ...
    if not ctx.is_scheduled():
        ...
        pmap(lambda node: node.wait_node_up(), nodes)
        logger.info("waiting a bit after validators started before starting traffic")
        time.sleep(10)
...
def run_remote_upload_file(ctx: CommandContext):
    ...
    pmap(
        lambda node: print_result(node, node.upload_file(ctx.args.src, ctx.args.dst)),
        targeted,
        on_exception="",
    )
...
def list_scheduled_cmds(ctx: CommandContext):
    ...
    logger.info(f"Getting schedule from {','.join([h.name() for h in targeted])}")
...
def build_parser():
    ...
    parser.add_argument(
        "--host-filter", type=str, help="Filter through the selected nodes using regex."
    )
    parser.add_argument(
        "--select-partition",
...
def register_schedule_subcommands(subparsers):
    ...
    list_subparsers = subparsers.add_parser("list", help="List all scheduled commands.")
...
def register_base_commands(subparsers):
    ...
    restart_parser = subparsers.add_parser(
        "restart-neard-runner", help="""Restarts the neard runner on all nodes."""
    )
    ...
    hard_reset_parser = subparsers.add_parser(
        "hard-reset", help="""Stops neard and clears all test state on all nodes."""
    )
    ...
    new_test_parser = subparsers.add_parser(
        "new-test",
        help="""
    ...
    new_test_parser.add_argument(
        "--gcs-state-sync",
        action="store_true",
        help="""Enable state dumper nodes to sync state to GCS. On localnet, it will dump locally.""",
    )
    new_test_parser.add_argument("--yes", action="store_true")
    new_test_parser.set_defaults(func=new_test_cmd)

    status_parser = subparsers.add_parser(
        "status", help="""Checks the status of test initialization on each node"""
    )
...
def register_subcommands(subparsers):
    ...
    update_config_parser = subparsers.add_parser(
        "update-config", help="""Update config.json with given flags for all nodes."""
    )
    ...
    start_traffic_parser = subparsers.add_parser(
        "start-traffic",
        help="Starts all nodes and starts neard mirror run on the traffic generator.",
    )
    start_traffic_parser.add_argument(
        "--batch-interval-millis",
        type=int,
        help="""Interval in millis between sending each mainnet block\'s worth of transactions.
    ...
    stop_parser = subparsers.add_parser("stop-nodes", help="kill all neard processes")
    ...
    amend_binaries_parsers = subparsers.add_parser(
        "amend-binaries",
        help="""
    ...
    amend_binaries_parsers.add_argument(
        "--neard-binary-url", type=str, required=True, help="URL to the neard binary."
    )
    ...
    run_cmd_parser = subparsers.add_parser(
        "run-cmd", help="""Run the cmd on the hosts."""
    )
