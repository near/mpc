...
def measure_tps_bps(nodes, tx_filename):
    ...
    measurement = mocknet.chain_measure_bps_and_tps(
        nodes[-1], input_tx_events[0], input_tx_events[-1]
    )
    result = {
        "bps": measurement["bps"],
        "in_tps": input_tps,
...
def check_tps(measurement, expected_in, expected_out=None, tolerance=0.05):
    ...
    return almost_equal(measurement["in_tps"], expected_in) and almost_equal(
...
def check_slow_blocks(initial_metrics, final_metrics):
    ...
    slow_process_blocks = (
        delta.block_processing_time["le +Inf"] - delta.block_processing_time["le 1"]
    )
    logger.info(f"Number of blocks processing for more than 1s: {slow_process_blocks}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--binary-url", required=False, help="url to download neard binary"
    )
    ...
    TOTAL_STAKE = 1000000
    bad_validator_stake = (
        int(TOTAL_STAKE * args.bad_stake / (100 * len(bad_validators)))
        if len(bad_validators) > 0
        else 0
    )
    good_validator_stake = int(
        TOTAL_STAKE * (100 - args.bad_stake) / (100 * len(good_validators))
    )
    ...
    if args.mode == "new":
        ...
        mocknet.create_and_upload_genesis_file_from_empty_genesis(
            ...
            [
                (
                    node,
                    bad_validator_stake * mocknet.ONE_NEAR
                    if get_role(node) == Role.BadValidator
                    else good_validator_stake * mocknet.ONE_NEAR,
                )
