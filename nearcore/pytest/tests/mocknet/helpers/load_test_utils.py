...
def function_call_set_state_then_delete_state(i, test_state, base_block_hash=None):
    ...
    if action == "add":
        ...
        logger.info(f"{account.key.account_id} set_state on {next_account_id} {tx_res}")
...
def meta_transaction_transfer(
    alice_account, test_state, base_block_hash, base_block_height
):
    ...
    signed_meta_tx = transaction.create_signed_delegated_action(
        ...
        receiver_account.key.account_id,
        [transfer_action],
        relayer_account.nonce + 2,
        base_block_height + 10**6,
        relayer_account.key.decoded_pk(),
        relayer_account.key.decoded_sk(),
    )
...
def random_transaction(i, test_state, base_block_hash, base_block_height):
    ...
    time.sleep(
        random.random()
        * test_state.num_test_accounts()
        / test_state.max_tps_per_node
        / 3
    )
    ...
    if choice == 0:
        ...
        function_call_ft_transfer_call(
            account, test_state, base_block_hash=base_block_hash
        )
    elif choice == 3:
        meta_transaction_transfer(
            account, test_state, base_block_hash, base_block_height
        )
...
def init_ft(node_account):
    tx_res = node_account.send_deploy_contract_tx("/home/ubuntu/fungible_token.wasm")
    ...
    tx_res = node_account.send_call_contract_raw_tx(
        node_account.key.account_id, "new_default_meta", s.encode("utf-8"), 0
    )
