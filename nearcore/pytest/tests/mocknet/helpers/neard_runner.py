...
class JSONHandler(http.server.BaseHTTPRequestHandler):
    def __init__(self, request, client_address, server):
        ...
        self.dispatcher.add_method(server.neard_runner.do_new_test, name="new_test")
        self.dispatcher.add_method(
            server.neard_runner.do_network_init, name="network_init"
        )
        self.dispatcher.add_method(
            server.neard_runner.do_update_config, name="update_config"
        )
        self.dispatcher.add_method(server.neard_runner.do_ready, name="ready")
        self.dispatcher.add_method(server.neard_runner.do_version, name="version")
        ...
        self.dispatcher.add_method(
            server.neard_runner.do_update_binaries, name="update_binaries"
        )
        self.dispatcher.add_method(
            server.neard_runner.do_make_backup, name="make_backup"
        )
        self.dispatcher.add_method(server.neard_runner.do_ls_backups, name="ls_backups")
        self.dispatcher.add_method(server.neard_runner.do_clear_env, name="clear_env")
        self.dispatcher.add_method(server.neard_runner.do_add_env, name="add_env")
    ...
    def do_POST(self):
        ...
        if l is None:
            self.send_error(http.HTTPStatus.BAD_REQUEST, "Content-Length missing")
            return
...
class NeardRunner:
    ...
    def _configure_neard_logs(self):
        ...
        logging.info(f"Setting log rotation policy in {self.logrotate_config_path}")
    ...
    def is_legacy(self):
        ...
        ) and os.path.exists(os.path.join(self.neard_home, "setup", "genesis.json")):
        ...
        if (
            ...
            and os.path.exists(os.path.join(target_dir, "data"))
            and os.path.exists(os.path.join(target_dir, "config.json"))
        ):
            return False
        sys.exit(
    ...
    def move_init_files(self):
        ...
        for path in paths:
            shutil.move(self.tmp_near_home_path(path), self.target_near_home_path(path))
        if not self.legacy_records:
            shutil.copyfile(
    ...
    def do_new_test(self, rpc_port=3030, protocol_port=24567, validator_id=None):
    ...
    def do_network_init(
        ...
        if not self.legacy_records and genesis_time is None:
            raise jsonrpc.exceptions.JSONRPCDispatchException(
                code=-32600,
                message="genesis_time argument required for nodes running via neard fork-network",
            )
    ...
    def do_start(self, batch_interval_millis=None):
        ...
        with self.lock:
            ...
            if state == TestState.STOPPED:
                if (
                    batch_interval_millis is not None
                    and not self.is_traffic_generator()
                ):
                    logging.warn(
                ...
                raise jsonrpc.exceptions.JSONRPCDispatchException(
                    code=-32600,
                    message="Cannot start node as test state has not been initialized yet",
                )
    ...
    def do_make_backup(self, backup_id, description=None):
        with self.lock:
            ...
            if state != TestState.RUNNING and state != TestState.STOPPED:
                raise jsonrpc.exceptions.JSONRPCDispatchException(
                    code=-32600, message="Cannot make backup as test state is not ready"
                )
    ...
    def update_binaries_url(self, neard_binary_url, epoch_height, binary_idx):
        if neard_binary_url is not None and (
            (epoch_height is None) != (binary_idx is None)
        ):
            logging.info(
        ...
        if epoch_height is not None:
            ...
            if binary:
                ...
                self.config["binaries"].append(
                    {"url": neard_binary_url, "epoch_height": epoch_height}
                )
                self.config["binaries"].sort(key=lambda binary: binary["epoch_height"])
        if binary_idx is not None:
            ...
            if binary_idx >= binaries_number:
                ...
                raise jsonrpc.exceptions.JSONRPCInvalidParams(
                    message=f"Invalid binary idx. Out of bounds for list of length {binaries_number}"
                )
    ...
    def do_update_binaries(self, neard_binary_url, epoch_height, binary_idx):
        with self.lock:
            ...
            if any(
                arg is not None for arg in [neard_binary_url, epoch_height, binary_idx]
            ):
                self.update_binaries_url(neard_binary_url, epoch_height, binary_idx)
                self.save_config()
            ...
            try:
                ...
                raise jsonrpc.exceptions.JSONRPCDispatchException(
                    code=-32603, message=f"Internal error downloading binaries: {e}"
                )
    ...
    def wanted_neard_path(self):
        ...
        try:
            r = requests.post(f"http://{self.data['neard_addr']}", json=j, timeout=5)
            r.raise_for_status()
    ...
    def get_env(self):
        ...
        return {
            ...
            **dotenv.dotenv_values(
                os.path.join(home_path, ".secrets")
    ...
    def poll_neard(self):
        if self.neard is not None:
            ...
            try:
                ...
                if int(p.create_time()) == self.data["neard_process"]["create_time"]:
    ...
    def source_near_home_path(self):
        if not self.is_traffic_generator():
            logging.warn("source_near_home_path() called on non-traffic-generator node")
    ...
    def get_start_traffic_generator_cmd(self, batch_interval_millis):
        ...
        if batch_interval_millis is not None:
            ...
            mirror_config_path = self.target_near_home_path("mirror-config.json")
    ...
    def check_amend_genesis(self):
        ...
        if path is None:
            ...
            if exit_code is not None and exit_code != 0:
                logging.error(f"neard amend-genesis exited with code {exit_code}")
                # for now just set the state to ERROR, and if this ever happens, the
                # test operator will have to intervene manually. Probably shouldn't
                # really happen in practice
                self.set_state(TestState.ERROR)
    ...
    def make_backup(self):
        ...
        for file in ["config.json", "genesis.json", "node_key.json"]:
            shutil.copyfile(
                self.target_near_home_path(file), os.path.join(backup_dir, file)
            )
    ...
    def make_initial_backup(self):
        ...
        self.making_backup(
            "start", description="initial test state after state root computation"
        )
        self.save_data()
        self.make_backup()
    ...
    def check_genesis_state(self):
        path, running, exit_code = self.poll_neard()
        if not running:
            logging.error(f"neard exited with code {exit_code} on the first run")
