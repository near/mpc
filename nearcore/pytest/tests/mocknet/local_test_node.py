...
class LocalTestNeardRunner:
    ...
    def start_neard_runner(self):
        ...
        with (
            open(self.home / "stdout", "ab") as stdout,
            open(self.home / "stderr", "ab") as stderr,
        ):
            args = [
                sys.executable,
                "tests/mocknet/helpers/neard_runner.py",
                "--home",
                self.home / "neard-runner",
                "--neard-home",
                self.home / ".near",
                "--neard-logs",
                self.home / "neard-logs",
                "--port",
                str(self.port),
            ]
...
def prompt_flags(args):
    ...
    if args.fork_height is not None:
        if not args.legacy_records:
            print("--legacy-records not given. Assuming it based on --fork-height")
...
def make_records(neard_binary_path, traffic_home_dir, start_height):
    ...
    shutil.copyfile(
        traffic_home_dir / "output/genesis.json",
...
def make_legacy_records(
    neard_binary_path, traffic_generator_home, node_homes, start_height
):
    make_records(neard_binary_path, traffic_generator_home / ".near", start_height)
    for node_home in node_homes:
        shutil.copyfile(
            traffic_generator_home / ".near/setup/genesis.json",
            node_home / ".near/setup/genesis.json",
        )
        shutil.copyfile(
            traffic_generator_home / ".near/setup/records.json",
...
def copy_source_home(source_home_dir, traffic_generator_home):
    shutil.copyfile(
        source_home_dir / "config.json", traffic_generator_home / "config.json"
    )
    shutil.copyfile(
        source_home_dir / "node_key.json", traffic_generator_home / "node_key.json"
    )
    shutil.copyfile(
        source_home_dir / "genesis.json", traffic_generator_home / "genesis.json"
    )
    try:
        shutil.copyfile(
            source_home_dir / "records.json", traffic_generator_home / "records.json"
        )
    except FileNotFoundError:
        pass
...
def get_nodes(local_mocknet_path=DEFAULT_LOCAL_MOCKNET_DIR):
    ...
    traffic_generator = NodeHandle(
        LocalTestNeardRunner(
            local_mocknet_path / "traffic-generator",
            runner_port,
            neard_rpc_port,
            neard_protocol_port,
        ),
    ...
    for home in node_homes:
        ...
        nodes.append(
            NodeHandle(
                LocalTestNeardRunner(
                    home, runner_port, neard_rpc_port, neard_protocol_port
                )
            )
        )
...
def local_test_setup_cmd(args):
    ...
    if args.legacy_records:
        ...
        make_legacy_records(
            neard_binary_path, traffic_generator_home, node_homes, args.fork_height
        )
    else:
        ...
        make_forked_network(
            neard_binary_path, setup_dir, node_homes, source_home_dir, target_home_dir
        )
    ...
    binaries_server = Server(
        addr=(binaries_server_addr, binaries_server_port), directory=binaries_path
    )
    ...
    node_config = {
        ...
        "binaries": [
            {
                "url": f"http://{binaries_server_addr}:{binaries_server_port}/neard",
                "epoch_height": 0,
            }
        ],
    }
...
if __name__ == "__main__":
    ...
    local_test_setup_parser = subparsers.add_parser(
        "local-test-setup",
        help="""
    ...
    local_test_setup_parser.add_argument(
        "--source-home-dir",
        type=str,
        help="""
