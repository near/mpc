...
class RemoteNeardRunner:
    ...
    def upload_neard_runner(self):
        self.node.machine.upload(
            "tests/mocknet/helpers/neard_runner.py",
            self.neard_runner_home,
            switch_user="ubuntu",
        )
        self.node.machine.upload(
            "tests/mocknet/helpers/requirements.txt",
            self.neard_runner_home,
            switch_user="ubuntu",
        )
    ...
    def upload_neard_runner_config(self, config):
        mocknet.upload_json(
            self.node, os.path.join(self.neard_runner_home, "config.json"), config
        )
    ...
    def run_cmd(
        ...
        if schedule_ctx is None:
            ...
            r = cmd_utils.schedule_cmd(
                self.node, cmd, schedule_ctx, raise_on_fail, return_on_fail
            )
        return r
    ...
    def init_python(self):
        cmd = (
            f"cd {self.neard_runner_home} && python3 -m virtualenv venv -p $(which python3)"
    ...
    def update_python(self):
        cmd = (
            f"cd {self.neard_runner_home} && ./venv/bin/pip install -r requirements.txt"
        )
        cmd_utils.run_cmd(self.node, cmd)

    def stop_neard_runner(self):
        self.node.machine.run(
            "sudo systemctl stop neard-runner;\
    ...
    def start_neard_runner(self):
        USER = "ubuntu"
        NEARD_RUNNER_CMD = (
            f'{self.neard_runner_home}/venv/bin/python {self.neard_runner_home}/neard_runner.py\
...
def get_traffic_generator_handle(traffic_generator):
    ...
    return NodeHandle(
        RemoteNeardRunner(traffic_generator, traffic_runner_home), can_validate=False
    )
