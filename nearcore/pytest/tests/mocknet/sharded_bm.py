...
def fetch_forknet_details(forknet_name, bm_params):
    ...
    if len(output) != num_cp_instances + 1:
        logger.error(f"Expected {num_cp_instances + 1} instances, got {len(output)}")
        sys.exit(1)
...
def handle_init(args):
    ...
    run_cmd_args = copy.deepcopy(args)
    run_cmd_args.host_filter = (
        f"({'|'.join(args.forknet_details['cp_instance_names'])})"
    )
...
def handle_apply_json_patches(args):
    ...
    base_genesis_patch = (
        f"{BENCHNET_DIR}/{args.case}/{args.bm_params['base_genesis_patch']}"
    )

    base_config_patch = (
        f"{BENCHNET_DIR}/{args.case}/{args.bm_params['base_config_patch']}"
    )
...
def stop_nodes(args, disable_tx_generator=False):
    ...
    stop_nodes_cmd_args.host_filter = (
        f"({'|'.join(args.forknet_details['cp_instance_names'])})"
    )
...
def start_nodes(args, enable_tx_generator=False):
    ...
    start_nodes_cmd_args.host_filter = (
        f"({'|'.join(args.forknet_details['cp_instance_names'])})"
    )
