...
def create_account(node, near_pk, near_sk):
    node.machine.upload(
        "tests/shardnet/scripts/create_account.sh", "/home/ubuntu", switch_user="ubuntu"
    )
    s = """
...
def restart_restaked(node, delay_sec, near_pk, near_sk, need_create_accounts):
    ...
    node.machine.upload(
        "tests/shardnet/scripts/restaked.sh", "/home/ubuntu", switch_user="ubuntu"
    )
    s = """
...
if __name__ == "__main__":
    ...
    pmap(
        lambda machine: restart_restaked(
            machine, delay_sec, near_pk, near_sk, need_create_accounts
        ),
        all_machines,
    )
