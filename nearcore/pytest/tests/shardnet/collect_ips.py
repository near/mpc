...
def visit_node(node_ip, timeout):
    ...
    account_id = None
    try:
        data = requests.get(f"http://{node_ip}:3030/status", timeout=timeout).json()
        ...
        data = requests.get(
            f"http://{node_ip}:3030/network_info", timeout=timeout
        ).json()
        peer_id_to_account_id = {}
        for known_producer in data["known_producers"]:
            peer_id_to_account_id[known_producer["peer_id"]] = known_producer[
                "account_id"
            ]
...
def discover_ips(node_ip, timeout):
    ...
    while next_to_visit:
        ...
        logger.info(
            f"Will visit {len(to_visit)} nodes. "
