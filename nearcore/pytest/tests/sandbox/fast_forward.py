...
nodes[0].json_rpc(
    "sandbox_fast_forward", {"delta_height": BLOCKS_TO_FASTFORWARD}, timeout=60
)
...
earliest = datetime.datetime.strptime(
    sync_info["earliest_block_time"][:-4], "%Y-%m-%dT%H:%M:%S.%f"
)
latest = datetime.datetime.strptime(
    sync_info["latest_block_time"][:-4], "%Y-%m-%dT%H:%M:%S.%f"
)

min_forwarded_secs = datetime.timedelta(0, BLOCKS_TO_FASTFORWARD * MIN_BLOCK_PROD_TIME)
max_forwarded_secs = datetime.timedelta(0, BLOCKS_TO_FASTFORWARD * MAX_BLOCK_PROD_TIME)
