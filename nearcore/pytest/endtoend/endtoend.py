...
def do_ping(account, rpc_server):
    ...
    logger.info(f"Sending 0 tokens from {account_id} to itself, tx result: {tx_res}")
...
if __name__ == "__main__":
    ...
    assert len(account_ids) == len(ips), (
        "List of test accounts must match the list of node IP addresses"
    )
    ...
    for ip, key in zip(ips, keys):
        nonce = mocknet_helpers.get_nonce_for_pk(
            key.account_id, key.pk, addr=rpc_server[0], port=rpc_server[1]
        )
        accounts.append(
            account_mod.Account(key, nonce, base_block_hash, rpc_infos=[(ip, port)])
        )
