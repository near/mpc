...
class EntityAPI:
    ...
    def get_trie_node(self, shard_uid, trie_node_hash):
        return self.query(
            "RawTrieNodeByHash", shard_uid=shard_uid, trie_node_hash=trie_node_hash
        )

    def get_trie_value(self, shard_uid, trie_value_hash):
        return self.query(
            "RawTrieValueByHash", shard_uid=shard_uid, trie_value_hash=trie_value_hash
        )
...
class EntityDataValue:
    ...
    def __getitem__(self, key):
        ...
        return EntityDataValue.of(
            [entry["value"] for entry in entries if entry["name"] == key][0]
        )
...
class TrieDiffer:
    ...
    def diff_tries(self, root_a: str, root_b: str):
        diffs = []
        iter_a = IterWithCurrent(
            TrieIterator(self.api_a, self.shard_uid, root_a).iterate()
        )
        iter_b = IterWithCurrent(
            TrieIterator(self.api_b, self.shard_uid, root_b).iterate()
        )
        while not iter_a.done or not iter_b.done:
            ...
            if path_a == path_b and kind_a == kind_b and value_a == value_b:
                print(
                    "\033[A\u001b[32m[Match] {} {} {}\u001b[0m".format(
                        path_a, kind_a, value_a
                    )
                )
                iter_a.next(False)
                iter_b.next(False)
                ...
                print(
                    "\033[A\u001b[31m[Mismatch] {} value {} <=> {}\u001b[0m\n".format(
                        path_a, value_a, value_b
                    )
                )
...
good_node_addr = input(
    "Enter the address of the good node, including port (e.g. 127.0.0.1:3030): "
)
stuck_node_addr = input(
    "Enter the address of the stuck node, including port (e.g. 127.0.0.1:3030): "
)
...
good_node.query(
    "RawTrieNodeByHash", shard_uid=shard_uid, trie_node_hash=good_state_root
)
print("valid.")
print("[Sanity check] Querying bad state root...", end="")
stuck_node.query(
    "RawTrieNodeByHash", shard_uid=shard_uid, trie_node_hash=stuck_state_root
)
print("valid.")
