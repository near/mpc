...
organization on GitHub to authenticate (<https://github.com/orgs/near/people>).
...
def _parse_args():
    ...
    default_test_path = pathlib.Path(__file__).parent.parent / DEFAULT_TEST_FILE
    ...
    parser.add_argument(
        "--branch", "-b", help="Branch to test. By default gets current one."
    )
    parser.add_argument(
        "--sha",
        "-s",
    ...
    group.add_argument(
        "--stdin", "-i", action="store_true", help="Read test set from standard input."
    )
    parser.add_argument(
        "--run-locally", "-l", action="store_true", help="Run tests locally."
    )
    parser.add_argument(
        "--dry-run",
        "-n",
...
def get_branch(sha: str = ""):
    if sha:
        return (
            subprocess.check_output(["git", "name-rev", sha], text=True)
            .strip()
            .split(" ")[-1]
        )
    else:
...
def read_tests_from_file(
    ...
    *,
    ...
    still recognize those includes.  Note that the line must start with `#./`,
    ...
    *,
    ...
    def impl(
        ...
        for lineno, line in enumerate(lines, start=1):
            line = line.rstrip()
            if line.startswith("./") or (include_comments and line.startswith("#./")):
                if depth == 4:
                    print(
                        f"{filename}:{lineno}: ignoring {line}; "
                        f"would exceed depth limit of {depth}"
                    )
                else:
                    inc_path = dirpath / line.lstrip("#")
                    yield from impl(
                        reader(inc_path).splitlines(),
                        inc_path,
                        inc_path.parent,
                        depth + 1,
...
def run_command(args, tests):
    ...
    try:
        ...
        styles = (colorama.Fore.RED, colorama.Fore.GREEN, colorama.Style.RESET_ALL)
    except ImportError:
        styles = ("", "", "")
    ...
    while True:
        ...
        if args.cancel:
            res = requests.post(
                NAYDUCK_BASE_HREF + f"/api/run/{args.cancel}/cancel",
                cookies={"nay-code": code},
            )
        else:
            res = requests.post(
                NAYDUCK_BASE_HREF + "/api/run/new",
                json=post,
                cookies={"nay-code": code},
            )
        if res.status_code != 401:
            break
        print(f"{styles[0]}Unauthorized.{styles[2]}\n")
        code = github_auth(code_path)
