...
# cspell:words getuid getgid nodocker
...
def docker_init(image, home_dir, init_flags):
    ...
    subprocess.check_output(
        [
            ...
            "%s:/srv/near" % home_dir,
            "-v",
            os.path.abspath("near/res") + ":/near/res",
            image,
            "near",
            "--home=/srv/near",
            "init",
...
def check_and_setup(
    nodocker, is_release, image, home_dir, init_flags, no_gas_price=False
):
    if nodocker:
    ...
    if os.path.exists(os.path.join(home_dir, "config.json")):
        ...
        if chain_id and genesis_config["chain_id"] != chain_id:
            ...
            exit(1)
        print(
            "Using existing node configuration from %s for %s"
            % (home_dir, genesis_config["chain_id"])
        )
        return
    ...
    if chain_id == "testnet":
        ...
        testnet_genesis_records = (
            "near/res/testnet_genesis_records_%s.json" % testnet_genesis_hash
        )
        ...
        init_flags.extend(
            [
                "--genesis-config",
                "near/res/testnet_genesis_config.json",
                "--genesis-records",
                testnet_genesis_records,
                "--genesis-hash",
                testnet_genesis_hash,
...
def docker_stop_if_exists(name):
    try:
        subprocess.Popen(
            ["docker", "stop", name], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        ).communicate()
    except subprocess.CalledProcessError:
        pass
    try:
        subprocess.Popen(
            ["docker", "rm", name], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        ).communicate()
    except subprocess.CalledProcessError:
        pass
...
def run_nodocker(home_dir, is_release, boot_nodes, telemetry_url, verbose):
    ...
    print("Auto update is not supported at the moment for runs outside of docker")
    ...
    if nodocker:
        ...
        subprocess.check_output(
            [
                ...
                USER,
                "-v",
                "%s:/srv/keypair-generator" % home,
                image,
                "keypair-generator",
                "--home=/srv/keypair-generator",
                "--generate-config",
                "node-key",
            ]
        )
...
def initialize_keys(
    home, is_release, nodocker, image, account_id, generate_signer_keys
):
    if nodocker:
        install_cargo()
...
def create_genesis(home, is_release, nodocker, image, chain_id, tracked_shards):
    ...
    if nodocker:
        cmd = [
            "./target/%s/genesis-csv-to-json" % ("release" if is_release else "debug")
        ]
        if home:
            cmd.extend(["--home", home])
        if chain_id:
...
# cspell:words stakewars
def start_stakewars(
    home, is_release, nodocker, image, telemetry_url, verbose, tracked_shards
):
    if nodocker:
        install_cargo()
    ...
    if nodocker:
        ...
        run_docker(
            image, home, boot_nodes="", telemetry_url=telemetry_url, verbose=verbose
        )
