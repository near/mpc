...
use crate::types::{MpcNetworkSetup, MpcParticipantSetup};
use mpc_contract::config::InitConfigV1;
use mpc_contract::primitives::{self, CandidateInfo, SignRequest};
...
impl NewMpcNetworkCmd {
    pub async fn run(&self, name: &str, rpc: Arc<NearRpcClients>) {
        ...
        let mut setup = OperatingDevnetSetup::load(rpc).await;
