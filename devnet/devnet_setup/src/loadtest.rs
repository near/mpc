...
impl NewLoadtestCmd {
    pub async fn run(&self, name: &str, client: Arc<NearRpcClients>) {
...
impl RunLoadtestCmd {
    pub async fn run(&self, name: &str, client: Arc<NearRpcClients>) {
        ...
        let setup = OperatingDevnetSetup::load(client.clone()).await;
        let loadtest_setup = setup
            .loadtest_setups
            .get(name)
            .expect(&format!("Loadtest setup with name {} does not exist", name));
