...
@pytest.mark.no_atexit_cleanup
def test_ckd_pv_request_lifecycle(shared_cluster: shared.MpcCluster):
    domains = shared_cluster.contract_state().get_running_domains()
    ...
    app_public_key_pv, app_private_key = ckd.generate_app_public_key_pv()
    path = "some_path"
    ckd_args = ckd.generate_ckd_pv_args(bls_domain, app_public_key_pv, path)
    tx = shared_cluster.request_node.sign_tx(
        shared_cluster.mpc_contract_account(),
        "request_app_private_key",
        ckd_args,
