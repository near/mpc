...
**[OT-based ECDSA](crates/threshold-signatures/docs/ecdsa/ot_based_ecdsa/intro.md)** (Secp256k1) — Originally derived from [Cait-Sith](https://github.com/cronokirby/cait-sith). Uses an offline phase with two protocols:
  ...
  - *Signing*: by default, three rounds that presign and sign in one computation over a pair of triples, whenever every live peer supports it; otherwise, or with `signature.online_presign` set to false, one round using a stored presignature.
