...
impl TeeAuthority {
    /// Generates a [`Attestation`] for this node, which can be used to send to the contract to prove that the node is running in a `TEE` context.
    ///
    /// Returns an [`anyhow::Error`] if a non-transient error occurs, that prevents the node from generating the attestation.
