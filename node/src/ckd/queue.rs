...
/// The minimum time that must elapse before we'll consider each ckd request for another attempt.
pub const CHECK_EACH_CKD_REQUEST_INTERVAL: Duration = Duration::seconds(1);
...
/// The number of blocks after which a ckd request is assumed to have timed out.
...
/// Maximum attempts we should make for each ckd when we are the leader.
const MAX_ATTEMPTS_PER_CKD_AS_LEADER: u64 = 10;

/// Manages the queue of ckd requests that still need to be handled.
/// The inputs to this queue are:
///  - Every block that comes from the indexer. For each block, we need the list of ckd
///    requests as well as the list of completed ckd requests (i.e. responses).
///  - The set of alive participants (nodes that we're currently connected to).
///  - The height of the indexer of each participant.
///
/// What this queue then provides, via `get_ckds_to_attempt`, is a list of ckd requests
/// that we should attempt to generate a ckd for. The list will be generated based on the
/// following goals:
///  - Assuming the network state is stable and nodes have consistent views of the connectivity and
///    indexer heights, each ckd request will be attempted by exactly one node ("leader").
///  - Each ckd request will be retried (by the current leader) if it has not been
...
///  - If a ckd request is too old so that it would have timed out on chain, it will be
///    discarded.
pub struct PendingCKDRequests {
    ...
    /// Map from ckd request ID to the request. Successful and expired requests are removed
    /// from this map. This is the "queue".
    pub(super) requests: HashMap<CKDId, QueuedCKDRequest>,

    /// See `RecentBlocksTracker`; allows us to decide what to do with each ckd request in
    /// the queue, as well as provide us a stream of finalized blocks from which we determine
    /// which ckd requests has been responded to.
    ...
    /// Recently completed ckd requests, for debugging purposes only.
    pub(super) recently_completed_requests: CompletedCKDRequests,
}

/// Block data to be buffered until the block is final.
#[derive(Clone)]
struct BufferedBlockData {
    ckd_requests: Vec<CKDId>,
    completed_requests: Vec<CKDId>,
...
impl Debug for BufferedBlockData {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if !self.ckd_requests.is_empty() {
            write!(
                f,
                "{} ckd reqs: {}",
                self.ckd_requests.len(),
                ellipsified_shortened_hash_list(&self.ckd_requests)
            )?;
        }
...
/// The state of a single ckd request in the queue.
pub(super) struct QueuedCKDRequest {
    ...
    /// This allows `get_ckds_to_attempt` to be called as frequently as desired.
    next_check_due: near_time::Instant,

    /// Progress of the computation of the ckd. Serves multiple purposes:
    ...
    pub computation_progress: Arc<Mutex<CKDComputationProgress>>,

    /// The current attempt to generate the ckd. This is weak to detect if the attempt has
    /// completed.
    pub active_attempt: Weak<CKDGenerationAttempt>,

    /// The time that the ckd was indexed.
    pub time_indexed: near_time::Instant,
}

/// Struct given to the ckd generation code.
pub struct CKDGenerationAttempt {
    /// The ckd request we should attempt to generate for.
    pub request: CKDRequest,
...
#[derive(Default)]
pub struct CKDComputationProgress {
    /// Number of attempts that have been made to generate the ckd.
...
impl QueuedCKDRequest {
    pub fn new(
        ...
        tracing::debug!(target: "ckd", "Leader selection order for request {:?} from block {}: {:?}", request.id, block_height, leader_selection_order);
        ...
        Self {
            ...
            computation_progress: Arc::new(Mutex::new(CKDComputationProgress::default())),
    ...
    /// Computes the leader selection order for a given ckd request.
    /// This will be a different pseudorandom order for each ckd request.
    fn leader_selection_order(
        ...
        let mut leader_selection_hashes = participants
            .iter()
            .map(|p| (Self::leader_selection_hash(p, ckd_request_id), *p))
            .collect::<Vec<_>>();
...
impl PendingCKDRequests {
    pub fn new(
        ...
        Self {
            ...
            network_api,
            recently_completed_requests: CompletedCKDRequests::default(),
        }
    }

    /// This must be called for every block that comes from the indexer.
    /// The requests are the ckd requests successfully submitted in the block, and the
    /// completed_requests are the ckds whose responses are included in the block.
    pub fn notify_new_block(
        &mut self,
        requests: Vec<CKDRequest>,
        completed_requests: Vec<CKDId>,
        block: &BlockViewLite,
    ) {
        let add_result = match self.recent_blocks.add_block(
            block,
            BufferedBlockData {
                ckd_requests: requests.iter().map(|r| r.id).collect(),
                completed_requests,
            ...
            Err(err) => {
                ...
                tracing::warn!(target: "ckd", "Ignoring block {:?} at height {}: {:?}", block.hash, block.height, err);
                return;
            }
        };

        metrics::MPC_PENDING_CKDS_QUEUE_BLOCKS_INDEXED.inc();
        for (final_block_height, buffered_block_data) in add_result.new_final_blocks {
            metrics::MPC_PENDING_CKDS_QUEUE_FINALIZED_BLOCKS_INDEXED.inc();
            ...
            for request_id in &buffered_block_data.completed_requests {
                tracing::debug!(target: "ckd", "Removing completed request {:?}", request_id);
                if let Some(request) = self.requests.remove(request_id) {
                    metrics::MPC_PENDING_CKDS_QUEUE_MATCHING_RESPONSES_INDEXED.inc();
                    ...
                    self.recently_completed_requests
                        .add_completed_request(CompletedCKDRequest {
        ...
        metrics::MPC_PENDING_CKDS_QUEUE_REQUESTS_INDEXED.inc_by(requests.len() as u64);
        for request in requests {
            self.requests
                .entry(request.id)
                .or_insert(QueuedCKDRequest::new(
                    &self.clock,
                    request.clone(),
                    block.hash,
                    block.height,
    ...
    /// Returns the set of participants that are eligible to be leaders for the ckd requests,
    ...
    /// Returns the list of ckd requests that we should attempt to generate a ckd for,
    /// right now, as the leader.
    ///
    /// The returned objects should only be dropped if:
    ///  - The ckd generation has failed. A retry can be issued immediately after that,
    ///    subject to a throttle of once per CHECK_EACH_CKD_REQUEST_INTERVAL.
    ///  - The ckd generation is successful, and the time that the response is submitted to
    ///    the chain has been written to the `CKDComputationProgress`.
    pub fn get_ckds_to_attempt(&mut self) -> Vec<Arc<CKDGenerationAttempt>> {
        let now = self.clock.now();

        let (eligible_leaders, maximum_height) = self.eligible_leaders_and_maximum_height();
        tracing::debug!(target: "ckd", "Eligible leaders: {:?}", eligible_leaders);
        self.recent_blocks
            .notify_maximum_height_available(maximum_height);

        let mut result = Vec::new();

        let mut ckds_to_remove = Vec::new();
        for (id, request) in &mut self.requests {
            if request.next_check_due > now {
                tracing::debug!(target: "ckd", "Skipping ckd request {:?} from block {} because it's not time yet", request.request.id, request.block_height);
                continue;
            }
            request.next_check_due = now + CHECK_EACH_CKD_REQUEST_INTERVAL;
            if request.active_attempt.strong_count() > 0 {
                // There's a current attempt to generate the ckd, so don't do anything.
                tracing::debug!(target: "ckd", "Skipping ckd request {:?} from block {} because there's already an active attempt", request.request.id, request.block_height);
                continue;
            }
            match self
                .recent_blocks
                .classify_block(request.block_hash, request.block_height)
            {
                ...
                | CheckBlockResult::Unknown => {
                    if let Some(leader) = request.current_leader(&eligible_leaders) {
                        tracing::debug!(target: "ckd", "Leader for ckd request {:?} from block {} is {}", request.request.id, request.block_height, leader);
                        if leader == self.my_participant_id {
                            {
                                let mut progress = request.computation_progress.lock().unwrap();
                                if progress.attempts >= MAX_ATTEMPTS_PER_CKD_AS_LEADER {
                                    tracing::debug!(target: "ckd", "Discarding ckd request {:?} from block {} because it has been attempted too many ({}) times", request.request.id, request.block_height, MAX_ATTEMPTS_PER_CKD_AS_LEADER);
                                    ckds_to_remove.push(*id);
                                    continue;
                                }
                                if progress.last_response_submission.is_some_and(|t| {
                                    now < t + MAX_LATENCY_BEFORE_EXPECTING_TRANSACTION_TO_FINALIZE
                                }) {
                                    tracing::debug!(target: "ckd", "Skipping ckd request {:?} from block {} because the last response was submitted too recently", request.request.id, request.block_height);
                                    continue;
                                }
                                progress.attempts += 1;
                            }
                            let attempt = Arc::new(CKDGenerationAttempt {
                ...
                CheckBlockResult::OptimisticButNotCanonical => {
                    ...
                    // the ckd.
                    tracing::debug!(target: "ckd", "Ignoring non-canonical ckd request {:?} from block {}", request.request.id, request.block_height);
                }
                CheckBlockResult::NotIncluded | CheckBlockResult::OlderThanRecentWindow => {
                    tracing::debug!(target: "ckd", "Discarding ckd request {:?} from block {}", request.request.id, request.block_height);
                    // This ckd request is definitely not useful anymore, so discard it.
                    ckds_to_remove.push(*id);
                }
            }
        }
        for id in ckds_to_remove {
        ...
        metrics::MPC_PENDING_CKDS_QUEUE_SIZE.set(self.requests.len() as i64);
        metrics::MPC_PENDING_CKDS_QUEUE_ATTEMPTS_GENERATED.inc_by(result.len() as u64);
        result
    }
...
#[cfg(test)]
mod tests {
    use super::{NetworkAPIForRequests, PendingCKDRequests, QueuedCKDRequest};
    use crate::ckd::queue::{
    ...
    fn test_ckd_request(
        ...
        loop {
            ...
            let leader_selection_order =
                QueuedCKDRequest::leader_selection_order(participants, request.id);
    ...
    fn test_pending_ckd_requests_leader_retry() {
        ...
        let mut pending_requests = PendingCKDRequests::new(
            clock.clock(),
            participants.clone(),
            my_participant_id,
            network_api.clone(),
        ...
        let to_attempt1 = pending_requests.get_ckds_to_attempt();
        ...
        assert_eq!(pending_requests.get_ckds_to_attempt().len(), 0);
        ...
        let to_attempt2 = pending_requests.get_ckds_to_attempt();
        ...
        drop(to_attempt1);
        assert_eq!(pending_requests.get_ckds_to_attempt().len(), 0);
        clock.advance(CHECK_EACH_CKD_REQUEST_INTERVAL);
        let to_attempt3 = pending_requests.get_ckds_to_attempt();
        ...
        let to_attempt4 = pending_requests.get_ckds_to_attempt();
        ...
        clock.advance(CHECK_EACH_CKD_REQUEST_INTERVAL);
        assert_eq!(pending_requests.get_ckds_to_attempt().len(), 1);
    ...
    fn test_pending_ckd_requests_abort_after_maximum_attempts() {
        ...
        for i in 0..MAX_ATTEMPTS_PER_CKD_AS_LEADER {
            let to_attempt = pending_requests.get_ckds_to_attempt();
