...
cargo build -p mpc-node --release              # Build node only

# Test
cargo test --profile test-release              # Unit and integration tests
cargo nextest run --cargo-profile=test-release --all-features  # With nextest

# Run single test
cargo test --profile test-release test_name
...
Uses Nix flake for reproducible environment: `nix develop`

Optional direnv setup for automatic activation (see README.md).

## CI Tools Required

- `cargo-nextest`, `cargo-sort`, `cargo-shear`, `cargo-deny`
- `zizmor` (GitHub Actions security)
- `ruff` (Python linting for pytest)
